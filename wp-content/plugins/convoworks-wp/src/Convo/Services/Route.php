...
use function Convoworks\OptimizePress\Support\array_get;
...
class Route
{
    ...
    public static function adminPage($label, $route, $callback, $options = [])
    {
        ...
        static::$adminPages[] = ['route' => $route, 'label' => $label, 'title' => array_get($options, 'title', $label), 'icon' => array_get($options, 'icon'), 'position' => array_get($options, 'position', 99), 'capability' => array_get($options, 'capability', 'manage_convoworks'), 'callback' => self::parseCallback($callback)];
    }
    /**
    ...
    {
        // Add page to collection
        static::$adminSubPages[] = ['route' => $route, 'parent' => $parent, 'label' => $label, 'title' => array_get($options, 'title', $label), 'position' => array_get($options, 'position', 999), 'capability' => array_get($options, 'capability', 'manage_convoworks'), 'callback' => self::parseCallback($callback), 'options' => $options];
    }
    /**
    ...
    public static function registerAdminPages()
    {
        ...
        if (static::$adminPages) {
            ...
            foreach ($adminPages as $adminPage) {
                add_menu_page(array_get($adminPage, 'label'), array_get($adminPage, 'title'), array_get($adminPage, 'capability'), array_get($adminPage, 'route'), array_get($adminPage, 'callback'), array_get($adminPage, 'icon'), array_get($adminPage, 'position'));
            }
        }
        // Then add sub pages
        if (static::$adminSubPages) {
            ...
            foreach (static::$adminSubPages as $adminSubPage) {
                ...
                if (!$route and $url) {
                    add_submenu_page(array_get($adminSubPage, 'parent'), array_get($adminSubPage, 'title'), array_get($adminSubPage, 'label'), array_get($adminSubPage, 'capability'), $url);
                } else {
                    add_submenu_page(array_get($adminSubPage, 'parent'), array_get($adminSubPage, 'title'), array_get($adminSubPage, 'label'), array_get($adminSubPage, 'capability'), $route, array_get($adminSubPage, 'callback'));
                }
            }
        }
