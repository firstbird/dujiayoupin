...
use Convoworks\Predis\Connection\Aggregate\PredisCluster;
...
use Convoworks\Symfony\Component\Cache\CacheItem;
use Convoworks\Symfony\Component\Cache\Exception\InvalidArgumentException;
use Convoworks\Symfony\Component\Cache\Exception\LogicException;
use Convoworks\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use Convoworks\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use Convoworks\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use Convoworks\Symfony\Component\Cache\Traits\RedisClusterProxy;
use Convoworks\Symfony\Component\Cache\Traits\RedisProxy;
use Convoworks\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \Convoworks\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \Convoworks\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            ...
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \Convoworks\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
