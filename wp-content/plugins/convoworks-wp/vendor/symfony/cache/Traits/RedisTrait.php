 ...
 */
namespace Convoworks\Symfony\Component\Cache\Traits;

use Convoworks\Predis\Command\Redis\UNLINK;
use Convoworks\Predis\Connection\Aggregate\ClusterInterface;
use Convoworks\Predis\Connection\Aggregate\RedisCluster;
use Convoworks\Predis\Connection\Aggregate\ReplicationInterface;
use Convoworks\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use Convoworks\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
use Convoworks\Predis\Response\ErrorInterface;
use Convoworks\Predis\Response\Status;
use Convoworks\Symfony\Component\Cache\Exception\CacheException;
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redis instanceof \Redis && !$redis instanceof \RedisArray && !$redis instanceof \RedisCluster && !$redis instanceof \Convoworks\Predis\ClientInterface && !$redis instanceof RedisProxy && !$redis instanceof RedisClusterProxy) {
        ...
        if ($redis instanceof \Convoworks\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\Convoworks\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\Convoworks\Predis\Client::class) && !\class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \Convoworks\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !\is_a($class, \Convoworks\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class)) {
        ...
        } elseif (\is_a($class, \Convoworks\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \Convoworks\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface)) {
    ...
    {
        if ($this->redis instanceof \Convoworks\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \Convoworks\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \Convoworks\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \Convoworks\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \Convoworks\Predis\ClientInterface ? $host->scan($cursor ?? 0, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, ?object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \Convoworks\Predis\ClientInterface && ($redis->getConnection() instanceof RedisCluster || $redis->getConnection() instanceof Predis2RedisCluster)) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \Convoworks\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \Convoworks\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \Convoworks\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \Convoworks\Predis\ClientInterface) {
            ...
            if (($connection instanceof ClusterInterface || $connection instanceof Predis2ClusterInterface) && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \Convoworks\Predis\Client($c);
                }
            }
