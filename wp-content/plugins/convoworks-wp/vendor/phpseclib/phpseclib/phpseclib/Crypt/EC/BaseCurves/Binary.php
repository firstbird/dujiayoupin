...
use Convoworks\phpseclib3\Math\BinaryField\Integer as BinaryInteger;
/**
 * Curves over y^2 + x*y = x^3 + a*x^2 + b
 *
 * @author  Jim Wigginton <terrafrost@php.net>
    ...
    public function setBasePoint($x, $y)
    {
        switch (\true) {
            case !\is_string($x) && !$x instanceof BinaryInteger:
                throw new \UnexpectedValueException('Convoworks\\Argument 1 passed to Binary::setBasePoint() must be a string or an instance of BinaryField\\Integer');
            case !\is_string($y) && !$y instanceof BinaryInteger:
                throw new \UnexpectedValueException('Convoworks\\Argument 2 passed to Binary::setBasePoint() must be a string or an instance of BinaryField\\Integer');
        }
        if (!isset($this->factory)) {
            throw new \RuntimeException('setModulo needs to be called before this method');
        }
