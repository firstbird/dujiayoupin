...
namespace Convoworks\PhpParser\NodeVisitor;
...
use Convoworks\PhpParser\Node\Name\FullyQualified;
use Convoworks\PhpParser\Node\Stmt;
use Convoworks\PhpParser\NodeVisitorAbstract;
class NameResolver extends NodeVisitorAbstract
{
    ...
    protected function resetState(Name $namespace = null)
    {
        ...
        $this->aliases = array(Stmt\Use_::TYPE_NORMAL => array(), Stmt\Use_::TYPE_FUNCTION => array(), Stmt\Use_::TYPE_CONSTANT => array());
    }
    protected function addAlias(Stmt\UseUse $use, $type, Name $prefix = null)
    {
        ...
        if (isset($this->aliases[$type][$aliasName])) {
            ...
            $this->errorHandler->handleError(new Error(\sprintf('Cannot use %s%s as %s because the name is already in use', $typeStringMap[$type], $name, $use->alias), $use->getAttributes()));
            return;
        }
