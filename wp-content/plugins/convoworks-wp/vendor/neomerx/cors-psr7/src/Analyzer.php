...
namespace Convoworks\Neomerx\Cors;
...
use Convoworks\Neomerx\Cors\Log\LoggerAwareTrait;
use Convoworks\Neomerx\Cors\Contracts\AnalyzerInterface;
use Convoworks\Neomerx\Cors\Contracts\Http\ParsedUrlInterface;
use Convoworks\Neomerx\Cors\Contracts\AnalysisResultInterface;
...
use Convoworks\Neomerx\Cors\Contracts\Constants\CorsRequestHeaders;
use Convoworks\Neomerx\Cors\Contracts\Constants\CorsResponseHeaders;
use Convoworks\Neomerx\Cors\Contracts\Constants\SimpleRequestHeaders;
use Convoworks\Neomerx\Cors\Contracts\Constants\SimpleRequestMethods;
/**
...
class Analyzer implements AnalyzerInterface
{
     ...
     * @var array
     */
    private $simpleMethods = [SimpleRequestMethods::GET => \true, SimpleRequestMethods::HEAD => \true, SimpleRequestMethods::POST => \true];
    /**
     * @var string[]
     */
    private $simpleHeadersExclContentType = [SimpleRequestHeaders::ACCEPT, SimpleRequestHeaders::ACCEPT_LANGUAGE, SimpleRequestHeaders::CONTENT_LANGUAGE];
    /**
    ...
    protected function analyzeImplementation(RequestInterface $request)
    {
        ...
        // check 'Host' request
        if ($this->strategy->isCheckHost() === \true && $this->isSameHost($request, $serverOrigin) === \false) {
        ...
        if ($requestOrigin === null) {
            $this->logInfo('Request is not CORS (request origin is empty).', ['request' => $requestOrigin, 'server' => $serverOrigin]);
            ...
            $this->logInfo('Request is not CORS (request origin equals to server one). ' . 'Check config settings for Server Origin.', ['request' => $requestOrigin, 'server' => $serverOrigin]);
            return $this->createResult(AnalysisResultInterface::TYPE_REQUEST_OUT_OF_CORS_SCOPE);
        }
        // #6.1.2 and #6.2.2
    ...
    protected function analyzeAsRequest(RequestInterface $request, ParsedUrlInterface $requestOrigin)
    {
        ...
        if ($this->strategy->isRequestCredentialsSupported($request) === \true) {
    ...
    protected function analyzeAsPreFlight(RequestInterface $request, ParsedUrlInterface $requestOrigin)
    {
        ...
        $headers[CorsResponseHeaders::VARY] = CorsRequestHeaders::ORIGIN;
        // #6.2.8
        if ($this->strategy->isPreFlightCanBeCached($request) === \true) {
        ...
        if ($isSimpleMethod === \false || $this->strategy->isForceAddAllowedMethodsToPreFlightResponse() === \true) {
        ...
        if ($isSimpleExclCT === \false || $this->strategy->isForceAddAllowedHeadersToPreFlightResponse() === \true) {
    ...
    protected function isSameHost(RequestInterface $request, ParsedUrlInterface $serverOrigin)
    {
        ...
        $isSameHost = $isHostUrlMatch === \true && ($serverOrigin->getPort() === null || $serverOrigin->getPort() === $portOrNull);
        return $isSameHost;
    }
    /**
     * @param ParsedUrlInterface $requestOrigin
    ...
    {
        $isSameOrigin = $requestOrigin->isHostEqual($serverOrigin) === \true && $requestOrigin->isPortEqual($serverOrigin) === \true && $requestOrigin->isSchemeEqual($serverOrigin) === \true;
        return $isSameOrigin;
    }
    /**
     * @param ParsedUrlInterface $requestOrigin
    ...
    {
        $origin = null;
        if ($request->hasHeader(CorsRequestHeaders::ORIGIN) === \true) {
    ...
    protected static function getFactory()
    {
        ...
        return new \Convoworks\Neomerx\Cors\Factory\Factory();
    }
    /**
     * @param RequestInterface $request
     *
    ...
    private function getRequestHostHeader(RequestInterface $request)
    {
        ...
        $host = empty($hostHeaderValue) === \true ? null : $hostHeaderValue[0];
        return $host;
    }
}
