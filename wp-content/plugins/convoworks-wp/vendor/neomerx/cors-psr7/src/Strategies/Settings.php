...
namespace Convoworks\Neomerx\Cors\Strategies;
...
use Convoworks\Neomerx\Cors\Contracts\Strategies\SettingsStrategyInterface;
/**
...
class Settings implements SettingsStrategyInterface
{
    ...
    {
        $this->settings[self::KEY_SERVER_ORIGIN] = \is_string($origin) === \true ? \parse_url($origin) : $origin;
        return $this;
    }
    /**
     * @inheritdoc
    ...
    public function isRequestOriginAllowed(ParsedUrlInterface $requestOrigin)
    {
        ...
        $isAllowed = isset($this->settings[self::KEY_ALLOWED_ORIGINS][CorsResponseHeaders::VALUE_ALLOW_ORIGIN_ALL]);
        if ($isAllowed === \false) {
    ...
    {
        $allSupported = \true;
        if (isset($this->settings[self::KEY_ALLOWED_HEADERS][self::VALUE_ALLOW_ALL_HEADERS]) === \true) {
            return $allSupported;
        }
        foreach ($headers as $header) {
            ...
            if (isset($this->settings[self::KEY_ALLOWED_HEADERS][$lcHeader]) === \false) {
                $allSupported = \false;
                $this->logInfo('Request header is not allowed. Check config settings for Allowed Headers.', ['header' => $header]);
                break;
            }
        }
        return $allSupported;
    ...
    protected function getDefaultSettings()
    {
        return [
             ...
             */
            self::KEY_SERVER_ORIGIN => [self::KEY_SERVER_ORIGIN_HOST => '', self::KEY_SERVER_ORIGIN_PORT => ParsedUrlInterface::DEFAULT_PORT],
            /**
    ...
    {
        return \array_key_exists($key, $this->settings) === \true ? $this->settings[$key] : $default;
    }
}
