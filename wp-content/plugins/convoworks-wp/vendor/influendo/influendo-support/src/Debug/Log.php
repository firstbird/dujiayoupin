...
namespace Convoworks\OptimizePress\Support\Debug;

use Closure;
use RuntimeException;
use InvalidArgumentException;
use Convoworks\Monolog\Handler\StreamHandler;
use Convoworks\Monolog\Handler\SyslogHandler;
use Convoworks\Monolog\Formatter\LineFormatter;
use Convoworks\Monolog\Handler\ErrorLogHandler;
use Convoworks\Monolog\Logger as MonologLogger;
use Convoworks\Monolog\Handler\RotatingFileHandler;
use function Convoworks\OPDashboard\opd_config;
class Log
{
    /**
     ...
     * @var array
     */
    protected static $levels = ['debug' => MonologLogger::DEBUG, 'info' => MonologLogger::INFO, 'notice' => MonologLogger::NOTICE, 'warning' => MonologLogger::WARNING, 'error' => MonologLogger::ERROR, 'critical' => MonologLogger::CRITICAL, 'alert' => MonologLogger::ALERT, 'emergency' => MonologLogger::EMERGENCY];
    /**
    ...
    public static function logger()
    {
        if (!static::$monolog) {
            ...
            // Determine log level
            if (\defined('WP_DEBUG') and WP_DEBUG === \true or \defined('OPD_DEBUG') and OPD_DEBUG === \true) {
    ...
    {
        static::$monolog->pushHandler($handler = new StreamHandler($path, $level));
    ...
    {
        static::$monolog->pushHandler($handler = new RotatingFileHandler($path, $days, $level));
        $handler->setFormatter(static::getDefaultFormatter());
    }
    /**
    ...
    {
        static::$monolog->pushHandler($handler = new ErrorLogHandler($messageType, $level));
        $handler->setFormatter(static::getDefaultFormatter());
    }
    /**
