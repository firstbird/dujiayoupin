...
namespace Convoworks\wapmorgan\Mp3Info;
...
class Mp3Info
{
     ...
     * @var array
     */
    private static $_vbrOffsets = [self::MPEG_1 => [21, 36], self::MPEG_2 => [13, 21], self::MPEG_25 => [13, 21]];
    /**
    ...
    private function parseAudio($filename, $fileSize, $mode)
    {
        ...
        \fclose($fp);
        $this->_parsingTime = \microtime(\true) - $time;
        return $audioSize;
    }
    /**
    ...
    private function readMpegFrame($fp)
    {
        ...
        if (!isset($header_bytes) || $header_bytes[0] !== 0xff || ($header_bytes[1] >> 5 & 0b111) != 0b111) {
            throw new \Exception('At ' . $pos . '(0x' . \dechex($pos) . ') should be a frame header!');
        }
    ...
    {
        $this->tags1['song'] = \trim(\fread($fp, 30));
        $this->tags1['artist'] = \trim(\fread($fp, 30));
        $this->tags1['album'] = \trim(\fread($fp, 30));
        $this->tags1['year'] = \trim(\fread($fp, 4));
        $this->tags1['comment'] = \trim(\fread($fp, 28));
        \fseek($fp, 1, \SEEK_CUR);
    ...
    private function readId3v2Body($fp)
    {
        ...
        if ($this->id3v2MajorVersion == 2) {
            ...
            $this->id3v2Flags = array('unsynchronisation' => (bool) \substr($flags, 0, 1), 'compression' => (bool) \substr($flags, 1, 1));
        } else {
            if ($this->id3v2MajorVersion == 3) {
                // parse id3v2.3.0 header flags
                $this->id3v2Flags = array('unsynchronisation' => (bool) \substr($flags, 0, 1), 'extended_header' => (bool) \substr($flags, 1, 1), 'experimental_indicator' => (bool) \substr($flags, 2, 1));
                if ($this->id3v2Flags['extended_header']) {
                    throw new \Exception('NEED TO PARSE EXTENDED HEADER!');
                }
            } else {
                if ($this->id3v2MajorVersion == 4) {
                    // parse id3v2.4.0 header flags
                    $this->id3v2Flags = array('unsynchronisation' => (bool) \substr($flags, 0, 1), 'extended_header' => (bool) \substr($flags, 1, 1), 'experimental_indicator' => (bool) \substr($flags, 2, 1), 'footer_present' => (bool) \substr($flags, 3, 1));
                    if ($this->id3v2Flags['extended_header']) {
                        throw new \Exception('NEED TO PARSE EXTENDED HEADER!');
                    }
    ...
    protected function parseId3v23Body($fp, $lastByte)
    {
        while (\ftell($fp) < $lastByte) {
            ...
            $this->id3v2TagsFlags[$frame_id] = array('flags' => array('tag_alter_preservation' => (bool) \substr($flags, 0, 1), 'file_alter_preservation' => (bool) \substr($flags, 1, 1), 'read_only' => (bool) \substr($flags, 2, 1), 'compression' => (bool) \substr($flags, 8, 1), 'encryption' => (bool) \substr($flags, 9, 1), 'grouping_identity' => (bool) \substr($flags, 10, 1)));
            switch ($frame_id) {
                ...
                case 'COMM':
                    ...
                    while (\ftell($fp) < $dataEnd) {
                        $char = \fgetc($fp);
                        if ($char == "\x00" && $actual_text === \false) {
                            if ($data['encoding'] == 0x1) {
                                # two null-bytes for utf-16
                                if ($last_null) {
                                    $actual_text = null;
                                } else {
                                    $last_null = \true;
                    ...
                    $this->tags2[$frame_id][$data['language']] = array('short' => (bool) ($data['encoding'] == 0x0) ? \mb_convert_encoding($short_description, 'utf-8', 'iso-8859-1') : \mb_convert_encoding($short_description, 'utf-8', 'utf-16'), 'actual' => (bool) ($data['encoding'] == 0x0) ? \mb_convert_encoding($actual_text, 'utf-8', 'iso-8859-1') : \mb_convert_encoding($actual_text, 'utf-8', 'utf-16'));
                    break;
    ...
    protected function parseId3v24Body($fp, $lastByte)
    {
        while (\ftell($fp) < $lastByte) {
            ...
            $frame_size = $data['frame_size'];
            $flags = \base_convert($data['flags'], 16, 2);
            $this->id3v2TagsFlags[$frame_id] = array('flags' => array('tag_alter_preservation' => (bool) \substr($flags, 1, 1), 'file_alter_preservation' => (bool) \substr($flags, 2, 1), 'read_only' => (bool) \substr($flags, 3, 1), 'grouping_identity' => (bool) \substr($flags, 9, 1), 'compression' => (bool) \substr($flags, 12, 1), 'encryption' => (bool) \substr($flags, 13, 1), 'unsynchronisation' => (bool) \substr($flags, 14, 1), 'data_length_indicator' => (bool) \substr($flags, 15, 1)));
            switch ($frame_id) {
                // case 'UFID':    # Unique file identifier
                //     break;
                ################# Text information frames
    ...
    private function handleTextFrame($frameSize, $raw)
    {
        ...
        switch ($data['encoding']) {
            ...
            case 0x1:
                # utf-16 with BOM
                return \mb_convert_encoding($data['information'] . "\x00", 'utf-8', 'utf-16');
    ...
    {
        foreach (['song' => 'TIT2', 'artist' => 'TPE1', 'album' => 'TALB', 'year' => 'TYER', 'comment' => 'COMM', 'track' => 'TRCK', 'genre' => 'TCON'] as $tag => $id3v2_tag) {
    ...
    public static function isValidAudio($filename)
    {
        ...
        return $raw === self::TAG2_SYNC || self::FRAME_SYNC === (\unpack('n*', $raw)[1] & self::FRAME_SYNC) || $filesize > 128 && \file_get_contents($filename, \false, null, -128, 3) === self::TAG1_SYNC;
    }
    /**
     * @param string $url
    ...
    public static function getUrlContentLength($url)
    {
        ...
        $head = \array_change_key_case(\get_headers($url, \true, $context));
