...
namespace Convoworks\Inpsyde\WPRESTStarter\Core\Request;
...
final class Request extends \WP_REST_Request implements ServerRequestInterface
{
    ...
    public static function from_wp_request(\WP_REST_Request $request) : Request
    {
        ...
        $instance = new self((string) $request->get_method(), (string) $request->get_route(), (array) ($request->get_attributes() ?? []));
    ...
    public function set_param($name, $value)
    {
        ...
        $this->http_message = \strtoupper($this->get_method()) === 'POST' ? $this->http_message->withParsedBody($this->get_body_params()) : $this->http_message->withQueryParams($this->get_query_params());
    }
    /**
    ...
    public function set_body($body)
    {
        ...
        $this->http_message = $this->http_message->withBody(stream_for($this->get_body()))->withParsedBody($this->get_body_params());
    }
    /**
    ...
    private function set_http_message_with_wp_headers()
    {
        ...
        $this->http_message = new PSR7Request($http_message->getMethod(), $http_message->getUri(), (array) ($this->get_headers() ?? []), $http_message->getBody(), $http_message->getProtocolVersion(), (array) ($http_message->getServerParams() ?? []));
    }
    /**
    ...
    private function set_http_message_with_wp_file_params()
    {
        ...
        if ($files) {
            $files = \array_map(function (array $file) {
                return new PSR7UploadedFile((string) ($file['tmp_name'] ?? $file['name'] ?? ''), (int) ($file['size'] ?? 0), (int) ($file['error'] ?? 0), $file['name'] ? (string) $file['name'] : null, $file['type'] ? (string) $file['type'] : null);
            }, $files);
        }
    ...
    private function set_wp_file_params_from_http_message()
    {
        ...
        if ($files) {
            $files = \array_map(function (UploadedFileInterface $file) {
                return ['name' => $file->getClientFilename(), 'type' => $file->getClientMediaType(), 'size' => $file->getSize(), 'tmp_name' => $file->getClientFilename(), 'error' => $file->getError()];
            }, $files);
        }
