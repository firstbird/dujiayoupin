...
namespace Convoworks\Inpsyde\WPRESTStarter\Core\Response;
...
final class Response extends \WP_REST_Response implements ResponseInterface
{
    ...
    public static function from_wp_response(\WP_HTTP_Response $response) : Response
    {
        ...
        $instance = new self($response->get_data(), (int) $response->get_status(), (array) ($response->get_headers() ?? []));
     ...
     * @return void
     */
    public function header($name, $value, $replace = \true)
    {
    ...
    private function set_http_message_with_wp_headers()
    {
        ...
        $this->http_message = new PSR7Response($http_message->getStatusCode(), $headers, $http_message->getBody(), $http_message->getProtocolVersion(), $http_message->getReasonPhrase());
    }
    /**
