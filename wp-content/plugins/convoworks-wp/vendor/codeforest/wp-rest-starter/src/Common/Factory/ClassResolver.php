...
namespace Convoworks\Inpsyde\WPRESTStarter\Common\Factory;

use Convoworks\Inpsyde\WPRESTStarter\Common\Factory\Exception\InvalidClass;
/**
...
class ClassResolver
{
    ...
    public function __construct(string $base, string $default_class = '')
    {
        ...
        if (!$this->base_is_class && !\interface_exists($base)) {
            throw new \InvalidArgumentException(__METHOD__ . ' requires a valid fully qualified class or interface name as first argument.');
        }
        if ($default_class) {
    ...
    public function resolve(string $class = '') : string
    {
        ...
        if (!$this->default_class) {
            throw new \InvalidArgumentException('Cannot resolve class name if no class is given and no default class is available.');
        }
    ...
    {
        if (!($this->base_is_class && $class === $this->base) && !\is_subclass_of($class, $this->base, \true)) {
            throw new InvalidClass("The class '{$class}' is invalid with respect to the defined base '{$this->base}'.");
        }
        return $class;
    }
}
