...
class BaseStub
{
    ...
    {
        if (!\method_exists('Convoworks\\Grpc\\ChannelCredentials', 'isDefaultRootsPemSet') || !ChannelCredentials::isDefaultRootsPemSet()) {
        ...
        if ($channel) {
            if (!\is_a($channel, 'Grpc\\Channel') && !\is_a($channel, 'Convoworks\\Grpc\\Internal\\InterceptorChannel')) {
    ...
    private static function updateOpts($opts)
    {
        ...
        if (\defined('Convoworks\\Grpc\\VERSION')) {
            $version_str = \Convoworks\Grpc\VERSION;
        } else {
    ...
    {
        if (\is_a($channel, 'Convoworks\\Grpc\\Internal\\InterceptorChannel')) {
            return function ($method, $argument, $deserialize, array $metadata = [], array $options = []) use($channel) {
