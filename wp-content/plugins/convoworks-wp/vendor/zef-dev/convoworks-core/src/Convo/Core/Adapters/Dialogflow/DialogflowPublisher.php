...
class DialogflowPublisher extends \Convo\Core\Publish\AbstractServicePublisher
{
    ...
    public function __construct($logger, \Convo\Core\IAdminUser $user, $serviceId, $serviceFactory, $serviceDataProvider, $convoServiceParamsFactory, $packageProviderFactory, $dialogflowApiFactory, $mediaService, $serviceReleaseManager, $platformPublishingHistory)
    {
    ...
    public function enable()
    {
        ...
        try {
            $avatar = $this->_mediaService->getMediaUrl($this->_serviceId, $config['dialogflow']['avatar']);
        } catch (\Exception $e) {
        ...
        if ($existingAgent !== null) {
            ...
            $supportedLocales = [];
            $agent = new Agent();
            $agent->setDisplayName($name)->setDefaultLanguageCode($defaultLocale)->setSupportedLanguageCodes($supportedLocales)->setTimeZone($defaultTimezone)->setDescription($desc)->setAvatarUri($avatar)->setEnableLogging(\true)->setMatchMode(MatchMode::MATCH_MODE_UNSPECIFIED)->setClassificationThreshold(0)->setApiVersion(ApiVersion::API_VERSION_V2)->setTier(Tier::TIER_STANDARD);
            $api->setAgent($agent);
            $export = $this->export();
    ...
    public function export()
    {
        ...
        $intents = \array_merge($intents, [$provider->getIntent('convo-core.DefaultWelcomeIntent')->getPlatformModel($this->getPlatformId()), $provider->getIntent('convo-core.DefaultFallbackIntent')->getPlatformModel($this->getPlatformId()), $provider->getIntent('convo-core.DefaultRepromptIntent')->getPlatformModel($this->getPlatformId())]);
    ...
    private function _buildIntent($name, $events, $isFallback, $utterances)
    {
        ...
        $intent = ['name' => $name, 'auto' => \true, 'contexts' => [], 'responses' => [['resetContexts' => \false, 'parameters' => $this->_buildParamsFromUtterances($unique)]], 'priority' => 500000, 'webhookUsed' => \true, 'webhookForSlotFilling' => \false, 'fallbackIntent' => $isFallback];
        if (!empty($events)) {
    ...
    private function _buildUtterance($convoUtterance)
    {
        ...
        for ($index = 0; $index < \count($parts); $index++) {
            ...
            if (isset($part['type'])) {
                ...
                try {
                    $sys_def = $provider->getEntity(\substr($part['type'], 1));
    ...
    private function _preparePropagateData()
    {
        ...
        return [PlatformPublishingHistory::DIALOGFLOW_AGENT_DEFAULT_LANGUAGE => $defaultLocale, PlatformPublishingHistory::DIALOGFLOW_AGENT_INTENT_MODEL_BYTES_SIZE => $dialogflowAgentIntentModelSize];
    }
}
