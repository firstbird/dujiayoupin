...
class DialogflowAuthService
{
    ...
    private function _createGoogleJwt($serviceAccount, $scope = 'https://www.googleapis.com/auth/dialogflow')
    {
        ...
        $now = \time();
        $expireTime = $now + 3600;
        $payload = \json_encode(["iss" => $serviceAccount['client_email'], "iat" => $now, "exp" => $expireTime, "scope" => $scope, "aud" => "https://oauth2.googleapis.com/token"]);
    ...
    {
        $tokenReq = $this->_httpFactory->buildRequest(IHttpFactory::METHOD_POST, $this->_httpFactory->buildUri(self::GOOGLE_TOKEN_URL), ['Content-Type' => 'application/x-www-form-urlencoded;charset=UTF-8'], $this->_toQuery(['grant_type' => 'urn:ietf:params:oauth:grant-type:jwt-bearer', 'assertion' => $this->_createGoogleJwt($this->_serviceAccount)]));
        $client = $this->_httpFactory->getHttpClient(['timeout' => 1000]);
        try {
            $res = $client->sendRequest($tokenReq);
            $response = \json_decode($res->getBody()->__toString(), \true);
            $this->_storeDialogflowAccessToken(\array_merge($response, ['created' => \time()]));
            return $response;
        } catch (\Exception $e) {
            //todo: temporary
            throw $e;
    ...
    {
        $userConfig = [$this->_platformId => [$this->_serviceAccount['project_id'] => ['access_token' => $data['access_token'], 'expires_in' => $data['expires_in'], 'created' => $data['created']]]];
