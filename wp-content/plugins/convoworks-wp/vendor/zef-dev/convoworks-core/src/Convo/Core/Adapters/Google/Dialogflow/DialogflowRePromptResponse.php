...
class DialogflowRePromptResponse extends DefaultTextCommandResponse
{
    ...
    {
        $text = $this->getText();
        $appResponse = array("payload" => array("google" => array("expectUserResponse" => !$this->shouldEndSession(), "richResponse" => array("items" => [array("simpleResponse" => array("textToSpeech" => "<speak><p>{$text}</p></speak>", "displayText" => $text))]))));
