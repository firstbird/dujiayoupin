...
class TestServiceRestHandler implements RequestHandlerInterface
{
    ...
    public function handle(\Psr\Http\Message\ServerRequestInterface $request) : \Psr\Http\Message\ResponseInterface
    {
        ...
        $route = $info->route('service-test/{serviceId}', \true);
        ...
        $exception = ["message" => null, "stack_trace" => null];
        try {
            $this->_logger->info('Running service instance [' . $service->getId() . '][' . $text_request . ']');
            $service->run($text_request, $text_response);
            $this->_eventDispatcher->dispatch(new ServiceRunRequestEvent(\true, $text_request, $text_response, $service, IPlatformPublisher::MAPPING_TYPE_DEVELOP), ServiceRunRequestEvent::NAME);
            if ($isStreaming) {
            ...
            $this->_eventDispatcher->dispatch(new ServiceRunRequestEvent(\true, $text_request, $text_response, $service, IPlatformPublisher::MAPPING_TYPE_DEVELOP, $e), ServiceRunRequestEvent::NAME);
    ...
    private function _getDebugInfo($service, $convoRequest, $exception = null)
    {
        ...
        $data = ['service_state' => $service->getServiceState(), 'variables' => ['service' => ['request' => $request_vars, 'session' => $session_vars, 'installation' => $installation_vars, 'user' => $user_vars], 'component' => $child_params], 'exception' => $exception];
        if (\is_a($convoRequest, '\\Convo\\Core\\Workflow\\IIntentAwareRequest')) {
            ...
            $data['intent'] = ['name' => $convoRequest->getIntentName(), 'slots' => $convoRequest->getSlotValues()];
        }
    ...
    private function _getChildData($service, $child)
    {
        ...
        $data = ['class' => (new \ReflectionClass($child))->getShortName()];
