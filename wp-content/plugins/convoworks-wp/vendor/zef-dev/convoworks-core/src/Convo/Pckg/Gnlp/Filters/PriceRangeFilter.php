...
class PriceRangeFilter extends \Convo\Pckg\Gnlp\Filters\AbstractFilter
{
    public static $DFAULTS = array('slot_name_min' => 'min_price', 'slot_name_max' => 'max_price', 'min_value' => 1000);
    ...
    public function visitToken(\Convo\Pckg\Gnlp\GoogleNlSyntaxToken $token)
    {
        if (\strtolower($token->getContent()) == 'from' || \strtolower($token->getContent() == 'between')) {
            ...
            if (\intval($min_val) < 1000) {
                $min_val = (string) \intval($min_val) * 1000;
            }
