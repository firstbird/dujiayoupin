...
class GetGeolocationElement extends \Convo\Core\Workflow\AbstractWorkflowContainerComponent implements \Convo\Core\Workflow\IConversationElement
{
    ...
    public function read(IConvoRequest $request, IConvoResponse $response)
    {
        if (\is_a($request, '\\Convo\\Core\\Adapters\\Alexa\\AmazonCommandRequest')) {
            ...
            $geolocationStatusVar = ['is_geolocation_supported' => $isGeoLocationSupported, 'is_geolocation_permission_granted' => $isGeoLocationPermissionGranted, 'is_location_sharing_enabled' => $isLocationSharingEnabled, 'exceeds_accuracy_threshold' => $exceedsAccuracyThreshold, 'is_data_fresh_enough' => $isDataFreshEnough, 'is_geolocation_available' => $isGeolocationAvailable, 'geolocation' => $geolocation];
            $this->_logger->debug('Printing geolocation status var [' . \print_r($geolocationStatusVar, \true) . ']');
            $selectedFlow = $this->_okFlow;
            if (!$isGeoLocationSupported || !$isGeoLocationPermissionGranted || $exceedsAccuracyThreshold || !$isDataFreshEnough || !$isGeolocationAvailable || !$isLocationSharingEnabled) {
