...
class ConversationBlock extends \Convo\Pckg\Core\Elements\ElementCollection implements \Convo\Core\Workflow\IPredispatchableBlock
{
    ...
    public function __construct($properties)
    {
        ...
        if (!isset($properties['role'])) {
            ...
            if ($this->_blockId === ConvoServiceInstance::BLOCK_TYPE_SESSION_START) {
                ...
                if ($this->_blockId === ConvoServiceInstance::BLOCK_TYPE_SESSION_END) {
                    $this->_role = IRunnableBlock::ROLE_SESSION_ENDED;
                } else {
                    if ($this->_blockId === ConvoServiceInstance::BLOCK_TYPE_SERVICE_PROCESSORS) {
                        $this->_role = IRunnableBlock::ROLE_SERVICE_PROCESSORS;
                    } else {
                        if ($this->_blockId === ConvoServiceInstance::BLOCK_TYPE_MEDIA_CONTROLS) {
    ...
    protected function _processProcessor(\Convo\Core\Workflow\IConvoRequest $request, \Convo\Core\Workflow\IConvoResponse $response, \Convo\Core\Workflow\IConversationProcessor $processor)
    {
