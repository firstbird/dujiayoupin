...
class AlexaSkillRestHandler implements RequestHandlerInterface
{
    ...
    public function handle(\Psr\Http\Message\ServerRequestInterface $request) : \Psr\Http\Message\ResponseInterface
    {
        ...
        if ($info->post() && ($route = $info->route('service-run/alexa-skill/{variant}/{serviceId}'))) {
            ...
            if ($info->post() && ($route = $info->route('service-run/amazon/{variant}/{serviceId}'))) {
                $variant = $route->get('variant');
                $serviceId = $route->get('serviceId');
            } else {
        ...
        if ($validationResult["verifiedSkillId"] === \false || $validationResult["verifiedRequestTimestamp"] === \false || $validationResult["validCertificateUrl"] === \false || $validationResult["verifiedCertificate"] === \false) {
    ...
    private function _handleAlexaSkillPathServiceIdPost(\Psr\Http\Message\ServerRequestInterface $request, $variant, $serviceId, $servicePlatformConfig)
    {
        ...
        try {
            $version_id = $this->_convoServiceFactory->getVariantVersion($owner, $serviceId, \Convo\Core\Adapters\Alexa\AmazonCommandRequest::PLATFORM_ID, $variant);
        ...
        if ($text_request->getIntentType() === 'SessionEndedRequest') {
            ...
            $text_response->prepareResponse(\Convo\Core\Adapters\Alexa\IAlexaResponseType::EMPTY_RESPONSE);
