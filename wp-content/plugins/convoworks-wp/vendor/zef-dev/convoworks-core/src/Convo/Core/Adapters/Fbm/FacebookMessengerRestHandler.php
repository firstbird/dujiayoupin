...
class FacebookMessengerRestHandler implements RequestHandlerInterface
{
    ...
    private function _handleAdminAuthPathFacebookPathServiceIdGet(\Psr\Http\Message\ServerRequestInterface $request, $serviceId)
    {
        $meta = $this->_convoServiceDataProvider->getServiceMeta(
            new AdminUser('', '', '', '', ''),
            // todo @tole why does this method need an AdminUser when it doesn't do anything with it?
            $serviceId
        );
        ...
        $this->_logger->info("Params [" . \print_r($params, \true) . "]");
    ...
    private function _handleRequest($variant, $serviceId, $request)
    {
        ...
        try {
            $version_id = $this->_convoServiceFactory->getVariantVersion($owner, $serviceId, \Convo\Core\Adapters\Fbm\FacebookMessengerCommandRequest::PLATFORM_ID, $variant);
        ...
        $servicePlatformConfig = $this->_convoServiceDataProvider->getServicePlatformConfig(new RestSystemUser(), $serviceId, IPlatformPublisher::MAPPING_TYPE_DEVELOP);
        $messenger_request = new \Convo\Core\Adapters\Fbm\FacebookMessengerCommandRequest($this->_logger, $serviceId, $request->getParsedBody());
        ...
        foreach ($messenger_request->getPlatformData()['entry'] as $entry) {
            ...
            $messenger_response = new \Convo\Core\Adapters\Fbm\FacebookMessengerCommandResponse();
            ...
            if ($delegation_nlp) {
                ...
                $this->_logger->info("Debug request with delegate [" . \print_r($messenger_request->getPlatformData(), \true) . "]");
            }
