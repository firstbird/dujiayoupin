...
class DefaultTextCommandResponse implements \Convo\Core\Workflow\IConvoResponse
{
    ...
    public function getPlatformResponse()
    {
        ...
        $response = ['text_responses' => \array_map(function ($item) {
            return $item;
        }, $this->_texts), 'text_reprompts' => \array_map(function ($item) {
            return $item;
        }, $this->_reprompts), 'should_end_session' => $this->shouldEndSession()];
        return $response;
    }
    // COMMON
