...
class CardElement extends \Convo\Core\Workflow\AbstractWorkflowContainerComponent implements \Convo\Core\Workflow\IConversationElement
{
    ...
    public function read(IConvoRequest $request, IConvoResponse $response)
    {
        ...
        $data = array("data_item_title" => $this->evaluateString($this->_dataItemTitle), "data_item_subtitle" => $this->evaluateString($this->_dataItemSubtitle), "data_item_description_1" => $this->evaluateString($this->_dataItemDescription1), "data_item_description_2" => $this->evaluateString($this->_dataItemDescription2), "data_item_description_3" => $this->evaluateString($this->_dataItemDescription3), "data_item_image_url" => $this->evaluateString($this->_dataItemImageUrl), "data_item_image_text" => $this->evaluateString($this->_dataItemImageText));
        $backButton = $this->evaluateString($this->_backButton);
        $this->_logger->debug('Back Button [' . $backButton . ']');
        $this->_logger->debug('Card element read method executed [' . \print_r($data, \true) . ']');
