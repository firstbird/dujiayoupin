...
abstract class AbstractMediaSourceContext extends \Convo\Core\Workflow\AbstractBasicComponent implements \Convo\Core\Workflow\IMediaSourceContext
{
    ...
    {
        $info = \Convo\Core\Workflow\IMediaSourceContext::DEFAULT_MEDIA_INFO;
        ...
        $model = $this->_getQueryModel();
        $info = \array_merge($info, ['count' => $this->getCount(), 'last' => $this->isLast(), 'first' => $model['post_index'] === 0, 'song_no' => $model['post_index'] + 1, 'loop_status' => $model['loop_status'], 'shuffle_status' => $model['shuffle_status'], 'playing' => $model['playing']]);
        $this->_logger->debug('Got current media info [' . \print_r($info, \true) . ']');
        return $info;
    }
    // QUERY
    /**
    ...
    protected function _getQueryModel()
    {
        ...
        if (empty($model)) {
            ...
            $model = ['playing' => \false, 'post_index' => 0, 'loop_status' => empty($this->_defaultLoop) ? \false : $this->getService()->evaluateString($this->_defaultLoop), 'shuffle_status' => empty($this->_defaultShuffle) ? \false : $this->getService()->evaluateString($this->_defaultShuffle), 'playlist' => [], 'song_offset' => 0, 'arguments' => []];
            $this->_saveQueryModel($model);
        }
        return $model;
    }
    ...
    {
        $this->_logger->debug('Saving query model [' . \print_r($model, \true) . '][' . $this . ']');
        $params = $this->getService()->getComponentParams(\Convo\Core\Params\IServiceParamsScope::SCOPE_TYPE_USER, $this);
