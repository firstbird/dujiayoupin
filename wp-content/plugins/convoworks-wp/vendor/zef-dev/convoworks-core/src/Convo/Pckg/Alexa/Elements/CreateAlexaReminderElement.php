...
class CreateAlexaReminderElement extends \Convo\Core\Workflow\AbstractWorkflowContainerComponent implements \Convo\Core\Workflow\IConversationElement
{
    ...
    public function read(IConvoRequest $request, IConvoResponse $response)
    {
        ...
        if (\is_a($request, AmazonCommandRequest::class)) {
            $payload = ['requestTime' => \date(self::REMINDER_SCHEDULE_DATE_FORMAT, \time()), 'trigger' => ['type' => self::SCHEDULED_ABSOLUTE]];
            if ($isRecurring) {
                if (!empty($recurrenceStartDate) && !empty($recurrenceStartTime) && !empty($recurrenceEndDate) && !empty($recurrenceEndTime)) {
            ...
            $supportedLocales = $this->_convoServiceDataProvider->getServiceMeta(new RestSystemUser(), $this->getService()->getId())['supported_locales'];
            // add spoken content for each supported locale
            $spokenContent = [];
            foreach ($supportedLocales as $supportedLocale) {
                $spokenContent[] = ['locale' => $supportedLocale, 'text' => $spokenInfoContentText, 'ssml' => '<speak>' . $spokenInfoContentText . '</speak>'];
            }
            $payload['alertInfo'] = ['spokenInfo' => ['content' => $spokenContent]];
