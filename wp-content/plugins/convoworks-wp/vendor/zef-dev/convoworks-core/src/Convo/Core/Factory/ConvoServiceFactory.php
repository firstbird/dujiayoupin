...
class ConvoServiceFactory
{
    ...
    public function __construct(\Psr\Log\LoggerInterface $logger, \Convo\Core\Factory\PackageProviderFactory $packageProviderFactory, \Convo\Core\IServiceDataProvider $convoServiceDataProvider)
    {
        $this->_logger = $logger;
    ...
    public function getService(\Convo\Core\IAdminUser $user, $serviceId, $versionId, $convoServiceParamsFactory)
    {
        ...
        $service = new \Convo\Core\ConvoServiceInstance($this->_logger, $eval, $convoServiceParamsFactory, $user, $serviceId);
