...
{
    const LOG_LEVELS = [LogLevel::EMERGENCY, LogLevel::ALERT, LogLevel::CRITICAL, LogLevel::ERROR, LogLevel::WARNING, LogLevel::NOTICE, LogLevel::INFO, LogLevel::DEBUG];
    private $_path;
    private $_prefix;
    private $_level;
    private $_useSession;
    ...
    public function log($level, $message, array $context = array())
    {
        ...
        $trace = @$backtrace[1];
        if (isset($backtrace[2]['class']) && \trim($backtrace[2]['class'])) {
    ...
    private function _logError($level, \Exception $error, $depth = 0)
    {
        ...
        $arr = \explode("\n", \trim($str));
        foreach ($arr as $item) {
            $this->log($level, \str_repeat("\t", $depth) . \trim($item));
        }
