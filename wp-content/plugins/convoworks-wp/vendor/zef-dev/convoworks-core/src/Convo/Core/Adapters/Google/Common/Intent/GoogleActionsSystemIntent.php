...
class GoogleActionsSystemIntent
{
    ...
    {
        $systemIntent = array("intent" => \Convo\Core\Adapters\Google\Common\Intent\IActionsIntent::OPTION, "data" => array("@type" => \Convo\Core\Adapters\Google\Common\Intent\IIntentInputValueDataSpec::OPTION_VALUE_SPEC));
    ...
    private function _prepareItemSelection($listProperties)
    {
        ...
        foreach ($listProperties["list_items"] as $listItem) {
            $obj = array("optionInfo" => array("key" => $listItem["list_item_key"], "synonyms" => [$listItem["list_item_title"]]), "title" => $listItem["list_item_title"]);
            ...
            if (!empty($listItem["list_item_image_url"]) && !empty($listItem["list_item_image_text"])) {
                $obj["image"] = array("url" => $listItem["list_item_image_url"], "accessibilityText" => $listItem["list_item_image_text"]);
            }
