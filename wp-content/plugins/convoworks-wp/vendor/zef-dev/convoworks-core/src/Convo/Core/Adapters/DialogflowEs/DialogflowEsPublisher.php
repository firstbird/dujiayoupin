...
class DialogflowEsPublisher extends \Convo\Core\Publish\AbstractServicePublisher
{
    ...
    public function __construct($logger, \Convo\Core\IAdminUser $user, $serviceId, $serviceFactory, $serviceDataProvider, $convoServiceParamsFactory, $packageProviderFactory, $dialogflowApiFactory, $serviceReleaseManager, $platformPublishingHistory)
    {
    ...
    public function enable()
    {
        ...
        $api = $this->_dialogflowApiFactory->getApi($this->_user, $this->_serviceId, $this->getPlatformId());
        ...
        if ($existingAgent !== null) {
            ...
            $config[$this->getPlatformId()]['time_created'] = \time();
            ...
            $supportedLocales = [];
            $agent = ["displayName" => $name, "defaultLanguageCode" => $defaultLocale, "supportedLanguageCodes" => $supportedLocales, "timeZone" => $defaultTimezone, "description" => $desc, 'enableLogging' => \true, 'classificationThreshold' => 0, 'apiVersion' => 2, 'tier' => 1];
            $api->setAgent($agent);
            $export = $this->export();
        ...
        if (isset($platform_config['mode']) && \strtolower($platform_config['mode']) === 'auto') {
            ...
            if (!isset($mapping['time_propagated']) || empty($mapping['time_propagated'])) {
                ...
                if ($mapping['time_propagated'] < $platform_config['time_updated']) {
                    ...
                    $workflowChanged = $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation($this->_serviceId, $this->getPlatformId(), PlatformPublishingHistory::DIALOGFLOW_AGENT, \json_encode($platform_config));
                    if ($workflowChanged) {
                        $changesCount++;
                    }
                }
                if ($mapping['time_propagated'] < $workflow['time_updated']) {
                    ...
                    $workflowChanged = $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation($this->_serviceId, $this->getPlatformId(), PlatformPublishingHistory::DIALOGFLOW_AGENT_INTENT_MODEL_BYTES_SIZE, $modelSize);
                    if ($workflowChanged) {
                        $changesCount++;
                    }
                }
                if ($mapping['time_propagated'] < $workflow['intents_time_updated']) {
                    ...
                    $intentsModelChanged = $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation($this->_serviceId, $this->getPlatformId(), PlatformPublishingHistory::DIALOGFLOW_AGENT_INTENT_MODEL_BYTES_SIZE, $modelSize);
                ...
                if ($mapping['time_propagated'] < $meta['time_updated']) {
                    ...
                    $metaChanged = $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation($this->_serviceId, $this->getPlatformId(), PlatformPublishingHistory::DIALOGFLOW_AGENT_DEFAULT_LANGUAGE, $meta['default_language']);
                    if ($metaChanged) {
                        $changesCount++;
                    }
                }
    ...
    public function export()
    {
        ...
        $data['.'][] = new SimpleFileResource('agent.json', 'application/json', \json_encode($this->_createAgentManifest($meta), $json_options_mask, 1024));
        $this->_logger->debug($data['.'][0]->getSize());
        $data['.'][] = new SimpleFileResource('package.json', 'application/json', \json_encode(['version' => '1.0.0'], $json_options_mask));
        foreach ($entities as $entity) {
            ...
            $definition = new SimpleFileResource($entity_name . '.json', 'application/json', \json_encode($built['definition'], $json_options_mask));
            $defaultLocale = $meta['default_language'];
            $entries = new SimpleFileResource($entity_name . '_entries_' . $defaultLocale . '.json', 'application/json', \json_encode($built['entries'], $json_options_mask));
            $data['entities'][] = $definition;
            $data['entities'][] = $entries;
        }
        $intents = \array_merge($intents, [$provider->getIntent('convo-core.DefaultFallbackIntent')->getPlatformModel($this->getPlatformId())]);
        foreach ($intents as $intent) {
            $userSaysByLanguage = $meta['default_language'];
            $built = $this->_buildIntents($intent);
            $this->_logger->debug('Final built intent [' . \print_r($built, \true) . ']');
            $intent_name = $built['intent']['name'];
            $definition = new SimpleFileResource($intent_name . '.json', 'application/json', \json_encode($built['intent'], $json_options_mask));
            $entries = new SimpleFileResource($intent_name . '_usersays_' . $userSaysByLanguage . '.json', 'application/json', \json_encode($built['usersays'], $json_options_mask));
            $data['intents'][] = $definition;
            $data['intents'][] = $entries;
        }
        $export = new ZipFileResource($this->_serviceId . '-' . $this->getPlatformId() . '.zip', 'application/zip', $data);
        return $export;
    }
    /**
    ...
    private function _buildIntents($intent)
    {
        ...
        $final = ['intent' => $intent_data, 'usersays' => $utterances];
        return $final;
    }
    private function _buildIntent($name, $events, $isFallback, $utterances)
    {
        ...
        $intent = ['name' => $name, 'auto' => \true, 'contexts' => [], 'responses' => [['resetContexts' => \false, 'parameters' => $this->_buildParamsFromUtterances($unique)]], 'priority' => 500000, 'webhookUsed' => \false, 'webhookForSlotFilling' => \false, 'fallbackIntent' => $isFallback];
        if (!empty($events)) {
    ...
    private function _buildParamsFromUtterances($utterances)
    {
        ...
        foreach ($utterances as $alias => $meta) {
            ...
            $params[] = ['required' => \false, 'dataType' => $meta, 'name' => $alias, 'value' => '$' . $alias, 'isList' => \false];
        }
        return $params;
    }
    /**
    ...
    {
        $utterance = ['data' => [], 'isTemplate' => \false, 'updated' => 0, 'count' => 0];
        $last_part = null;
        $next_part = null;
        ...
        for ($index = 0; $index < \count($parts); $index++) {
            ...
            if (isset($part['type'])) {
                ...
                if (isset($last_part['type'])) {
                    $utterance['data'][] = ['text' => ' ', 'userDefined' => \false];
                }
            } else {
    ...
    {
        $id = StrUtil::uuidV4();
        $definition = ['id' => $id, 'name' => $convoEntity->getName(), 'isOverridable' => \true, 'isEnum' => \false, 'isRegexp' => \false, 'automatedExpansion' => \false, 'allowFuzzyExtraction' => \false];
        $entries = [];
        foreach ($convoEntity->getValues() as $value) {
            $entries[] = ['value' => $value->getValue(), 'synonyms' => $value->getSynonims()];
        }
    ...
    {
        $id = StrUtil::uuidV4();
        $definition = ['id' => $id, 'name' => $convoEntityName, 'isOverridable' => \true, 'isEnum' => \false, 'isRegexp' => \false, 'automatedExpansion' => \false, 'allowFuzzyExtraction' => \false];
        $entries = [];
        foreach ($catalog->getCatalogValues($this->getPlatformId()) as $value) {
            $entries[] = ['value' => $value, 'synonyms' => [$value]];
        }
        return ['definition' => $definition, 'entries' => $entries];
    }
    // UTIL
    ...
    {
        $platform_config = $this->_convoServiceDataProvider->getServicePlatformConfig($this->_user, $this->_serviceId, IPlatformPublisher::MAPPING_TYPE_DEVELOP);
        $df_config = $platform_config[$this->getPlatformId()];
        $agent = ['name' => $df_config['name'] ?? 'DefaultName', 'description' => $df_config['description'] ?? 'Demo agent', 'language' => $meta['default_language'], 'supportedLanguages' => [], 'disableInteractionLogs' => \false, 'disableStackdriverLogs' => \true, 'defaultTimezone' => $df_config['default_timezone'] ?? 'Europe/Madrid', 'webhook' => ['url' => '', 'available' => \false, 'useForDomains' => \false, 'cloudFunctionsEnabled' => \false, 'cloudFunctionsInitialized' => \false], 'isPrivate' => \true, 'customClassifierMode' => 'disabled', 'mlMinConfidence' => 0.0, 'onePlatformApiVersion' => 'v2', 'analyzeQueryTextSentiment' => \false, 'dialogflowBuilderMode' => \false];
        return $agent;
    }
    ...
    public function createRelease($platformId, $targetReleaseType, $targetReleaseStage, $alias, $versionId = null)
    {
        ...
        return ["delegation_nlp_id" => $this->getPlatformId(), "delegation_nlp_data" => ["agent_version" => $createdDialogflowAgentVersion, "agent_environment" => $environment]];
    }
    public function createVersionTag($platformId, $versionTagId = null)
    {
        ...
        return ["delegation_nlp_id" => $this->getPlatformId(), "delegation_nlp_data" => ["agent_version" => $createdDialogflowAgentVersion]];
    }
    public function importToDevelop($platformId, $fromAlias, $toAlias, $versionId = null, $versionTag = null)
    {
        ...
        $environment = $api->loadVersionIntoEnvironment('-', $versionNumber, \true);
        return ["delegation_nlp_id" => $this->getPlatformId(), "delegation_nlp_data" => ["agent_version" => $existingAgent, "agent_environment" => $environment]];
    }
    public function importToRelease($platformId, $targetReleaseType, $targetReleaseStage, $alias, $versionId = null, $nextVersionId = null)
    {
        ...
        return ["delegation_nlp_id" => $this->getPlatformId(), "delegation_nlp_data" => ["agent_version" => $agentVersion, "agent_environment" => $environment]];
    }
    private function _preparePropagateData()
    {
        ...
        $platform_config = $config[$this->getPlatformId()] ?? '';
        return [PlatformPublishingHistory::DIALOGFLOW_AGENT => \json_encode($platform_config), PlatformPublishingHistory::DIALOGFLOW_AGENT_DEFAULT_LANGUAGE => $defaultLocale, PlatformPublishingHistory::DIALOGFLOW_AGENT_INTENT_MODEL_BYTES_SIZE => $dialogflowAgentIntentModelSize];
    }
}
