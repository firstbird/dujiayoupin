...
class GetInSkillProductsElement extends AbstractWorkflowComponent implements IConversationElement
{
    ...
    private function _getInSkillProducts(AmazonCommandRequest $request)
    {
        ...
        $productsUri = $this->_httpFactory->buildUri($platformData['context']['System']['apiEndpoint'] . self::GET_ISP_PRODUCTS, $this->_getQueryParams());
        if ($shouldGetProductById) {
            $productId = $this->evaluateString($this->_productId);
            $productsUri = $this->_httpFactory->buildUri($platformData['context']['System']['apiEndpoint'] . self::GET_ISP_PRODUCTS . '/' . $productId);
        }
        $this->_logger->debug('Products URI [' . $productsUri . ']');
        $ispProductsApiRequest = $this->_httpFactory->buildRequest(IHttpFactory::METHOD_GET, $productsUri->__toString(), ['Accept-Language' => $request->getLocale(), 'Authorization' => 'Bearer ' . $platformData['context']['System']['apiAccessToken']]);
        $res = $client->sendRequest($ispProductsApiRequest);
        return \json_decode($res->getBody()->__toString(), \true);
    }
