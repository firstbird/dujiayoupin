...
class PlatformPublisherFactory
{
    ...
    public function __construct($publicRestBaseUrl, $logger, $serviceFactory, $serviceDataProvider, $serviceParamsFactory, $mediaService, $amazonPublishingService, $dialogflowApiFactory, $facebookMessengerApiFactory, $viberApi, $packageProviderFactory, $adminUserDataProvider, $serviceReleaseManager, $platformPublishingHistory)
    {
    ...
    public function getPublisher(\Convo\Core\IAdminUser $user, $serviceId, $platformId)
    {
        ...
        if ($platformId === \Convo\Core\Adapters\Alexa\AmazonCommandRequest::PLATFORM_ID) {
            return new \Convo\Core\Adapters\Alexa\AlexaSkillPublisher($this->_publicRestBaseUrl, $this->_logger, $owner, $serviceId, $this->_convoServiceFactory, $this->_convoServiceDataProvider, $this->_convoServiceParamsFactory, $this->_amazonPublishingService, $this->_packageProviderFactory, $this->_adminUserDataProvider, $this->_serviceReleaseManager, $this->_mediaService, $this->_platformPublishingHistory);
        }
        if ($platformId === \Convo\Core\Adapters\Google\Dialogflow\DialogflowCommandRequest::PLATFORM_ID) {
            return new \Convo\Core\Adapters\Dialogflow\DialogflowPublisher($this->_logger, $owner, $serviceId, $this->_convoServiceFactory, $this->_convoServiceDataProvider, $this->_convoServiceParamsFactory, $this->_packageProviderFactory, $this->_dialogflowApiFactory, $this->_mediaService, $this->_serviceReleaseManager, $this->_platformPublishingHistory);
        }
        if ($platformId === 'dialogflow_es') {
            return new \Convo\Core\Adapters\DialogflowEs\DialogflowEsPublisher($this->_logger, $owner, $serviceId, $this->_convoServiceFactory, $this->_convoServiceDataProvider, $this->_convoServiceParamsFactory, $this->_packageProviderFactory, $this->_dialogflowApiFactory, $this->_serviceReleaseManager, $this->_platformPublishingHistory);
        }
        if ($platformId === \Convo\Core\Adapters\Google\Gactions\ActionsCommandRequest::PLATFORM_ID) {
            return new \Convo\Core\Adapters\Google\Common\GactionsPublisher($this->_logger, $owner, $serviceId, $this->_convoServiceDataProvider, $this->_serviceReleaseManager);
        }
        if ($platformId === \Convo\Core\Adapters\Fbm\FacebookMessengerCommandRequest::PLATFORM_ID) {
            return new \Convo\Core\Adapters\Fbm\FacebookMessengerServicePublisher($this->_logger, $owner, $serviceId, $this->_facebookMessengerApiFactory, $this->_convoServiceDataProvider, $this->_serviceReleaseManager, $this->_platformPublishingHistory);
        }
        if ($platformId === \Convo\Core\Adapters\Viber\ViberCommandRequest::PLATFORM_ID) {
            return new \Convo\Core\Adapters\Viber\ViberServicePublisher($this->_logger, $owner, $serviceId, $this->_viberApi, $this->_convoServiceDataProvider, $this->_serviceReleaseManager, $this->_platformPublishingHistory, $this);
        }
        if ($platformId === \Convo\Core\Adapters\ConvoChat\DefaultTextCommandRequest::PLATFORM_ID) {
            return new \Convo\Core\Adapters\ConvoChat\ConvoChatServicePublisher($this->_logger, $owner, $serviceId, $this->_convoServiceDataProvider, $this->_serviceReleaseManager, $this);
        }
    ...
    private function _getServiceOwner($user, $serviceId)
    {
        ...
        $meta = $this->_convoServiceDataProvider->getServiceMeta($user, $serviceId, \Convo\Core\Publish\IPlatformPublisher::MAPPING_TYPE_DEVELOP);
        if (!empty($meta["owner"])) {
            $owner = $this->_adminUserDataProvider->findUser($meta["owner"]);
            $this->_logger->debug('Going to return owner [' . $owner->getUsername() . '][' . $owner->getEmail() . '] of the service [' . $serviceId . ']');
        } else {
