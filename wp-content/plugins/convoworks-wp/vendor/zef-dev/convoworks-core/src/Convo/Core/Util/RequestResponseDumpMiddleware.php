        ...
        try {
            ...
            if (!empty($this->_convoDumpConfig) && \in_array($handlerId, $this->_convoDumpConfig["classIdentifiers"])) {
                if (!empty($this->_convoDumpConfig["dumpPath"])) {
                    ...
                    $dataToLog = ["requestBodyToLog" => \json_decode($requestBodyToLog), "responseBodyToLog" => \json_decode($responseToLog)];
                    $this->_storeData($dataToLog, $this->_convoDumpConfig["dumpPath"], $handlerId, $this->_convoDumpConfig["dumpLimitPairs"] * 2);
                }
            }
            ...
            if (!empty($this->_convoErrorDumpConfig["errorDumpPath"])) {
                ...
                $requestBodyToLog = $body->getContents();
                $body->rewind();
                $dataToLog = ["requestBodyToLog" => \json_decode($requestBodyToLog, \true)];
                $dataToLog['requestBodyToLog']['convoErrorReport'] = array('errorMessage' => $t->getMessage(), 'inFileAtLine' => $t->getFile() . '(' . $t->getLine() . ')', 'trace' => $t->getTraceAsString());
                $this->_storeData($dataToLog, $this->_convoErrorDumpConfig["errorDumpPath"], $handlerId, $this->_convoErrorDumpConfig["errorDumpLimit"]);
            }
