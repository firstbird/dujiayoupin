...
class MigrateTo1 extends \Convo\Core\Migrate\AbstractMigration
{
        ...
        foreach ($serviceData['blocks'] as $i => $block_data) {
            if (\strpos($block_data['properties']['block_id'], '_read_') === 0) {
                if (!empty($block_data['properties']['elements'])) {
                    ...
                    $serviceData['subroutines'][] = ['class' => 'Convoworks\\Adm\\Alexax\\Elem\\ElementsSubroutine', 'namespace' => 'convo-core', 'properties' => ['subroutine_id' => $block_data['properties']['block_id'], 'name' => $block_data['properties']['block_id'], 'description' => '', 'elements' => $block_data['properties']['elements'], '_component_id' => $block_data['properties']['_component_id'], '_workflow' => 'read']];
                }
                if (!empty($block_data['properties']['processors'])) {
                    ...
                    $serviceData['subroutines'][] = ['class' => 'Convoworks\\Adm\\Alexax\\Proc\\ProcessorSubroutine', 'namespace' => 'convo-core', 'properties' => ['subroutine_id' => $block_data['properties']['block_id'], 'name' => $block_data['properties']['block_id'], 'description' => '', 'processor' => $block_data['properties']['processors'][0], '_component_id' => $block_data['properties']['_component_id'], '_workflow' => 'process']];
                }
    ...
    protected function _migrateComponent($componentData)
    {
        ...
        if (!isset($componentData['namespace'])) {
            ...
            if ($componentData['class'] && \strpos($componentData['class'], 'Convoworks\\Adm\\Nlp') !== \false) {
        ...
        if ($componentData['class'] === '\\Adm\\Alexax\\Elem\\ReadBlockElement') {
            ...
            $componentData['class'] = 'Convoworks\\Adm\\Alexax\\Elem\\ReadElementsSubroutine';
        ...
        if ($componentData['class'] === '\\Adm\\Alexax\\Proc\\VirtualProcessor') {
            ...
            $componentData['class'] = 'Convoworks\\Adm\\Alexax\\Proc\\ProcessProcessorSubroutine';
            $componentData['properties']['subroutine_id'] = $componentData['properties']['block_id'];
            unset($componentData['properties']['block_id']);
        }
