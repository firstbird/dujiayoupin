...
class ActionsCommandResponse extends DefaultTextCommandResponse
{
    ...
    {
        $appResponse = array("conversationToken" => $this->_conversationToken, "expectUserResponse" => !$this->shouldEndSession(), "expectedInputs" => array());
    ...
    private function _prepareSimpleResponse($response)
    {
        if ($this->shouldEndSession()) {
            $response["finalResponse"]["richResponse"] = array("items" => array($this->_responseElement->getSimpleResponseElement("Conversation Response", $this->getTextSsml(), $this->getText())));
            $response['expectUserResponse'] = \false;
        } else {
            $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array("items" => array($this->_responseElement->getSimpleResponseElement("Conversation Response", $this->getTextSsml(), $this->getText())));
        }
    ...
    private function _prepareMediaResponse($response)
    {
        ...
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array(
            "items" => array($this->_responseElement->getSimpleResponseElement("Music Response Text", $ssml, $text), $this->_responseElement->getMediaResponseElement($mp3Url, $fileNameFromMp3Url)),
    ...
    {
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array("items" => array(
            ...
            array("name" => "Basic Card Response", "basicCard" => array("title" => "Some title", "subtitle" => "Some sub title", "formattedText" => "This is a basic card.  Text in a basic card can include \"quotes\" and\nmost other unicode characters including emoji 📱.  Basic cards also support\nsome markdown formatting like *emphasis* or _italics_, **strong** or\n__bold__, and ***bold itallic*** or ___strong emphasis___ as well as other\nthings like line  \nbreaks", "image" => array("url" => "https://icatcare.org/app/uploads/2018/06/Layer-1704-1920x840.jpg", "accessibilityText" => "This is an image of an image"), "buttons" => array(array("title" => "This is a button", "openUrlAction" => array("url" => "https://assistant.google.com/"))))),
        ));
        $response["expectedInputs"][0]["possibleIntents"] = $this->_intentResolver->resolveIntent(IActionsIntent::TEXT);
        return $response;
    }
    ...
    private function _prepareCarouselBrowseResponse($response)
    {
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array("items" => array(
            ...
            array("name" => "Carousel Browse Response", "carouselBrowse" => array("items" => array(array("title" => "Test", "description" => "Sampe", "footer" => "One more", "openUrlAction" => array("url" => "https://google.com"), "image" => array("url" => "https://image.insider.com/5d02563ddaa4821bf4575092?width=1100&format=jpeg&auto=webp", "accessibilityText" => "Information about Google Assistant")), array("title" => "Test", "description" => "Sampe", "footer" => "One more", "openUrlAction" => array("url" => "https://google.com"), "image" => array("url" => "https://i.redd.it/w85fwvz6ttrz.jpg", "accessibilityText" => "Information about Google Assistant"))))),
        ));
        $response["expectedInputs"][0]["possibleIntents"] = $this->_intentResolver->resolveIntent(IActionsIntent::TEXT);
        return $response;
    }
    ...
    private function _prepareTableCardResponse($response)
    {
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array("items" => array(
            ...
            // todo map the properties from an external element
            array("name" => "Music Response with Carousel Browse", "tableCard" => array("title" => "Test", "subtitle" => "Sub Test", "image" => array("url" => "https://www.humanesociety.org/sites/default/files/styles/768x326/public/2018/08/kitten-440379.jpg?h=f6a7b1af&itok=vU0J0uZR", "accessibilityText" => "Information about Google Assistant"), "columnProperties" => array(array("header" => "Header 1", "horizontalAlignment" => "LEADING"), array("header" => "Header 2", "horizontalAlignment" => "CENTER"), array("header" => "Header 3", "horizontalAlignment" => "TRAILING")), "rows" => array(array("cells" => array(array("text" => "test 1 1"), array("text" => "test 2 1"), array("text" => "test 3 1"))), array("cells" => array(array("text" => "test 1 2"), array("text" => "test 2 2"), array("text" => "test 3 2"))), array("cells" => array(array("text" => "test 1 3"), array("text" => "test 2 3"), array("text" => "test 3 3"))), array("cells" => array(array("text" => "test 1 4"), array("text" => "test 2 4"), array("text" => "test 3 4")))), "buttons" => array(array("title" => "This is a button", "openUrlAction" => array("url" => "https://assistant.google.com/"))))),
        ));
        $response["expectedInputs"][0]["possibleIntents"] = $this->_intentResolver->resolveIntent(IActionsIntent::TEXT);
        return $response;
    }
    ...
    {
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"]["items"][0] = $this->_responseElement->getSimpleResponseElement("Account Linking Prompt", $this->getTextSsml(), $this->getText());
        $response["expectedInputs"][0]["possibleIntents"] = $this->_intentResolver->resolveIntent(IActionsIntent::SIGN_IN, IIntentInputValueDataSpec::SIGN_IN_VALUE_SPEC, $this->getText());
        return $response;
    }
    /**
    ...
    {
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array("items" => array($this->_responseElement->getSimpleResponseElement("Confirmation Response", "PLACEHOLDER", "PLACEHOLDER", IActionsIntent::CONFIRMATION)));
        $response["expectedInputs"][0]["possibleIntents"] = $this->_intentResolver->resolveIntent(IActionsIntent::CONFIRMATION, IIntentInputValueDataSpec::CONFIRMATION_VALUE_SPEC, array("requestConfirmationText" => $this->_confirmationText));
        return $response;
    }
    /**
     * Will be used later as a visual response for google devices with displays.
    ...
    {
        $response["expectedInputs"][0]["inputPrompt"]["richInitialPrompt"] = array("items" => array(array("simpleResponse" => array(
            // todo set carousel text
            "textToSpeech" => "Carousel",
        ))));
        $optionValues = array(array("optionInfo" => array("key" => "SELECTION_KEY_GOOGLE_HOME", "synonyms" => array("synonym of title 1", "synonym of title 2", "synonym of title 3")), "description" => "Item 1 description", "image" => array("url" => "https://icatcare.org/app/uploads/2018/06/Layer-1704-1920x840.jpg", "accessibilityText" => "This is an image of an image"), "title" => "Item 1 Title"));
        $response["expectedInputs"][0]["possibleIntents"] = $this->_intentResolver->resolveIntent(IActionsIntent::OPTION, IIntentInputValueDataSpec::OPTION_VALUE_SPEC, $optionValues);
        return $response;
    }
    /**
