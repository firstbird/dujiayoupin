...
class GoogleActionsElements
{
    ...
    {
        $simpleResponse = array("name" => $name, "simpleResponse" => array("ssml" => $ssml, "displayText" => $displayText));
        if ($intent === IActionsIntent::CONFIRMATION) {
            $simpleResponse = array("name" => "Confirmation Response", "simpleResponse" => array("ssml" => $ssml, "displayText" => $displayText));
        }
        return $simpleResponse;
    }
    public function getBasicCardResponseElement($value)
    {
        $basicCardBody = array("formattedText" => $value["data_item_description_1"]);
        ...
        if (!empty($value["data_item_image_url"]) && !empty($value["data_item_image_text"])) {
            $basicCardBody["image"] = array("url" => $value["data_item_image_url"], "accessibilityText" => $value["data_item_image_text"]);
        }
    ...
    public function getCarouselBrowseResponseElement($value)
    {
        ...
        foreach ($value["browse_carousel_items"] as $browseCarouselItem) {
            $obj = array("title" => $browseCarouselItem["browse_carousel_item_title"]);
            ...
            if (!empty($browseCarouselItem["browse_carousel_item_image_url"]) && !empty($browseCarouselItem["browse_carousel_item_image_text"])) {
                $obj["image"] = array("url" => $browseCarouselItem["browse_carousel_item_image_url"], "accessibilityText" => $browseCarouselItem["browse_carousel_item_image_text"]);
            }
            if (!empty($browseCarouselItem["browse_carousel_item_url"])) {
                $obj["openUrlAction"] = array("url" => $browseCarouselItem["browse_carousel_item_url"]);
            }
    ...
    {
        return array("simpleResponse" => array("textToSpeech" => "<speak>{$text}</speak>"));
    }
    /**
    ...
    public function getSuggestionsElement($suggestions)
    {
        ...
        foreach ($suggestions as $suggestion) {
            $suggestionObject = array("title" => $suggestion);
    ...
    {
        return array("name" => "Music Response with Media", "mediaResponse" => array("mediaType" => "AUDIO", "mediaObjects" => array(array("contentUrl" => $mp3Url, "name" => $fileNameFromMp3Url, "description" => $mp3Url, "largeImage" => array("url" => "https://zef.dev/images/zefdev-800x650.png", "accessibilityText" => "Zef Dev")))));
    }
