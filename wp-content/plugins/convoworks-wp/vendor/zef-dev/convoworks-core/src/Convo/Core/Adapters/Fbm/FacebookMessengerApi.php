...
use Convoworks\Facebook\Facebook;
class FacebookMessengerApi
{
    ...
    {
        $this->_serviceConfig = $this->_convoServiceDataProvider->getServicePlatformConfig($this->_user, $this->_serviceId, IPlatformPublisher::MAPPING_TYPE_DEVELOP);
        $this->_fb = new Facebook(['app_id' => $this->_serviceConfig['facebook_messenger']['app_id'], 'app_secret' => $this->_serviceConfig['facebook_messenger']['app_secret']]);
    }
    public function callSendApi($senderPSID, $response)
    {
        $requestBody = ["recipient" => ["id" => $senderPSID], "message" => $response];
    ...
    {
        $requestBody = ["get_started" => ["payload" => "GET_STARTED"], "greeting" => [["locale" => "default", "text" => "Hi {{user_full_name}} and welcome to " . $this->_serviceIdToName($this->_serviceId)]]];
        $uri = $this->_httpFactory->buildUri(self::FACEBOOK_MESSENGER_PROFILE_URL, ['access_token' => $this->_serviceConfig['facebook_messenger']['page_access_token']]);
        $request = $this->_httpFactory->buildRequest("POST", $uri, [], $requestBody);
        $client = $this->_httpFactory->getHttpClient();
        $client->sendRequest($request);
    }
    public function callSubscriptionsApi($callbackUrl)
    {
        ...
        $requestBody = ['object' => 'page', 'callback_url' => $callbackUrl, 'fields' => $this->_provideWebhookEvents(), 'include_values' => 'true', 'verify_token' => $this->_serviceConfig['facebook_messenger']['webhook_verify_token']];
    ...
    public function callSubscribedApps()
    {
        ...
        $requestBody = ['subscribed_fields' => $this->_provideWebhookEvents()];
