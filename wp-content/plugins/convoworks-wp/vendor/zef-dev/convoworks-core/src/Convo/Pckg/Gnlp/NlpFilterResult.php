...
class NlpFilterResult extends \Convo\Core\Workflow\DefaultFilterResult
{
    ...
    public function setSlotTokenValue($name, $value, \Convo\Pckg\Gnlp\GoogleNlSyntaxToken $token)
    {
        if (!isset($this->_tokenValues[$name])) {
            $this->_tokenValues[$name] = new \Convo\Pckg\Gnlp\SlotValue();
        }
        $this->_tokenValues[$name]->addTokenValue($value, $token);
        $this->setSlotValue($name, $value);
    }
    public function getSlotTokenValue($name, \Convo\Pckg\Gnlp\GoogleNlSyntaxToken $token)
    {
        if (!isset($this->_tokenValues[$name])) {
    ...
    public function getClosestTokens(\Convo\Pckg\Gnlp\NlpFilterResult $result)
    {
    ...
    public function getTokensAtDistance(\Convo\Pckg\Gnlp\NlpFilterResult $result, $targetDistance)
    {
        /** @var GoogleNlSyntaxToken[] $tokens */
        /** @var GoogleNlSyntaxToken[] $other_tokens */
        /** @var GoogleNlSyntaxToken[] $this_tokens */
    ...
    public function getDistance(\Convo\Pckg\Gnlp\NlpFilterResult $result)
    {
    ...
    {
        if (\in_array($token, $this->_tokens, \true)) {
            return;
        }
        $this->_tokens[] = $token;
    }
