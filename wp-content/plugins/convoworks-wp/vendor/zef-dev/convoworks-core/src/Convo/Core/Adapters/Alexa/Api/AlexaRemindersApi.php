...
class AlexaRemindersApi extends \Convo\Core\Adapters\Alexa\Api\AlexaApi
{
    ...
    public function getReminder(AmazonCommandRequest $request, string $id)
    {
        try {
            ...
            switch ($e->getCode()) {
                case 401:
                    throw new \Convo\Core\Adapters\Alexa\Api\InsufficientPermissionsGrantedException('Missing [alexa::alerts:reminders:skill:read] permissions.', null, $e);
                case 404:
    ...
    public function createReminder(AmazonCommandRequest $request, array $payload)
    {
        try {
            ...
            switch ($e->getCode()) {
                case 400:
                    throw new InvalidRequestException('Request body [' . \print_r($payload, \true) . '] is invalid.', null, $e);
                case 401:
                    throw new \Convo\Core\Adapters\Alexa\Api\InsufficientPermissionsGrantedException('Missing [alexa::alerts:reminders:skill:write] permissions.', null, $e);
                default:
                    throw new \Exception('Something went wrong, please try later again.', null, $e);
            }
        }
