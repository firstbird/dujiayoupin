...
class ActionsCommandRequest implements IConvoRequest, LoggerAwareInterface
{
    ...
    public function init()
    {
        ...
        $this->_conversationToken = isset($this->_data['conversation']['conversationToken']) ? $this->_data['conversation']['conversationToken'] : self::DEFAULT_CONVERSATION_TOKEN;
        $this->_accessToken = $this->_data['user']['accessToken'] ?? null;
        switch ($this->_data['inputs'][0]['intent']) {
            case IActionsIntent::MAIN:
                $this->_isLaunch = \true;
                if (isset($this->_data['inputs'][0]['arguments'][0]['name']) && $this->_data['inputs'][0]['arguments'][0]['name'] == 'trigger_query') {
    ...
    {
        return \get_class($this) . '[' . self::PLATFORM_ID . '][' . $this->_serviceId . '][' . $this->_text . '][' . $this->_installationId . ']' . '[' . $this->_sessionId . '][' . $this->_isLaunch . '][' . $this->_isEnd . ']';
    }
    /**
     * @inheritDoc
     */
