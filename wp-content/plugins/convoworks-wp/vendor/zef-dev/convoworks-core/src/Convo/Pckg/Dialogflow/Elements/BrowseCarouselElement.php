...
class BrowseCarouselElement extends \Convo\Core\Workflow\AbstractWorkflowContainerComponent implements \Convo\Core\Workflow\IConversationElement
{
    ...
    public function read(IConvoRequest $request, IConvoResponse $response)
    {
        ...
        for ($i = $start; $i < $end; ++$i) {
            $val = $items[$i];
            $params->setServiceParam($slot_name, ['value' => $val, 'index' => $i, 'natural' => $i + 1, 'first' => $i === $start, 'last' => $i === $end]);
            \array_push($browseCarouselItems, array("browse_carousel_item_key" => $this->evaluateString(\strval($i)), "browse_carousel_item_title" => $this->evaluateString($this->_browseCarouselItemTitle), "browse_carousel_item_description" => $this->evaluateString($this->_browseCarouselItemDescription), "browse_carousel_item_footer" => $this->evaluateString($this->_browseCarouselItemFooter), "browse_carousel_item_image_url" => $this->evaluateString($this->_browseCarouselItemImageUrl), "browse_carousel_item_image_text" => $this->evaluateString($this->_browseCarouselItemImageText), "browse_carousel_item_url" => $this->evaluateString($this->_browseCarouselItemUrl)));
        }
        $data = array("browse_carousel_items" => $browseCarouselItems);
        $this->_logger->debug('List element read method executed [' . \print_r($data, \true) . ']');
