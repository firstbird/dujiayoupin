...
class OpenTDBTriviaAdapterElement extends \Convo\Core\Workflow\AbstractWorkflowContainerComponent implements \Convo\Core\Workflow\IConversationElement
{
    ...
    public function read(IConvoRequest $request, IConvoResponse $response)
    {
        ...
        $uri = $this->_httpFactory->buildUri(self::BASE_URL, ['amount' => $this->evaluateString($this->_amount), 'category' => $this->evaluateString($this->_category), 'type' => 'multiple']);
        $this->_logger->info('Final URI [' . $uri . ']');
        $res = $http_client->sendRequest($this->_httpFactory->buildRequest(IHttpFactory::METHOD_GET, $uri));
        ...
        $result = \json_decode($res->getBody()->__toString(), \true);
        $this->_logger->info('Got response trivia [' . \print_r($result, \true) . ']');
        $questions = [];
        foreach ($result['results'] as $item) {
            ...
            foreach ($possible_answers as $index => $possible_answer) {
                $cw_answers[] = ['text' => $this->_decodeSpecialChars($possible_answer), 'letter' => self::LETTERS[$index], 'is_correct' => $possible_answer === $item['correct_answer']];
                if ($cw_answers[$index]['is_correct']) {
                    $correct = $cw_answers[$index];
                }
            }
            $questions[] = ['text' => $this->_decodeSpecialChars($item['question']), 'answers' => $cw_answers, 'correct_answer' => $correct];
        }
