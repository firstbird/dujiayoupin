...
use Convoworks\Middlewares\Utils\Traits\HasResponseFactory;
...
class TrailingSlash implements MiddlewareInterface
{
     ...
     */
    public function redirect(bool $redirect = \true) : self
    {
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler) : ResponseInterface
    {
        ...
        if ($this->redirect && $uri->getPath() !== $path) {
            return $this->createResponse(301)->withHeader('Location', (string) $uri->withPath($path));
        }
