<?php
/**
 * 精确修复 app-modern.min.js 中的 JSON.parse(data) 问题
 * 这个脚本会避免重复修复，只修复真正需要修复的地方
 */
...
echo "开始精确修复 JSON.parse(data) 问题...\n";
echo "文件路径: $file_path\n\n";
...
// 定义需要修复的模式和替换内容
$fixes = [
    // 模式1: 单独的 data = JSON.parse(data); 行
    [
        'pattern' => '/^(\s+)data\s*=\s*JSON\.parse\(data\);/m',
        'replacement' => '$1// 修复：检查 data 是否已经是对象，如果是则直接使用，否则尝试解析
$1if (typeof data === \'string\') {
$1    try {
$1        data = JSON.parse(data);
$1    } catch (e) {
$1        console.error(\'JSON.parse 失败:\', e, \'原始数据:\', data);
$1        return;
$1    }
$1}',
        'description' => '修复单独的 data = JSON.parse(data); 行'
    ],
    
    // 模式2: 单独的 _data = JSON.parse(data); 行
    [
        'pattern' => '/^(\s+)_data\s*=\s*JSON\.parse\(data\);/m',
        'replacement' => '$1// 修复：检查 data 是否已经是对象，如果是则直接使用，否则尝试解析
$1if (typeof data === \'string\') {
$1    try {
$1        _data = JSON.parse(data);
$1    } catch (e) {
$1        console.error(\'JSON.parse 失败:\', e, \'原始数据:\', data);
$1        return;
$1    }
$1} else {
$1    _data = data; // data 已经是对象，直接使用
$1}',
        'description' => '修复单独的 _data = JSON.parse(data); 行'
    ],
    
    // 模式3: var _data = JSON.parse(data); 行
    [
        'pattern' => '/^(\s+)var\s+_data\s*=\s*JSON\.parse\(data\);/m',
        'replacement' => '$1// 修复：检查 data 是否已经是对象，如果是则直接使用，否则尝试解析
$1var _data;
$1if (typeof data === \'string\') {
$1    try {
...
$1    _data = data; // data 已经是对象，直接使用
$1}',
        'description' => '修复 var _data = JSON.parse(data); 行'
    ],
    
    // 模式4: var data = JSON.parse(data); 行
    [
        'pattern' => '/^(\s+)var\s+data\s*=\s*JSON\.parse\(data\);/m',
        'replacement' => '$1// 修复：检查 data 是否已经是对象，如果是则直接使用，否则尝试解析
$1if (typeof data === \'string\') {
$1    try {
$1        data = JSON.parse(data);
...
$1    }
$1}',
        'description' => '修复 var data = JSON.parse(data); 行'
    ]
];
...
if ($content !== $original_content) {
    // 创建备份
    $backup_path = $file_path . '.backup-v2.' . date('Y-m-d-H-i-s');
