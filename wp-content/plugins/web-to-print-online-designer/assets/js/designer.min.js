...
function addFontFaceToFaric(name, type, url) {
    ...
    if (type == "google") {
        // mzl mod fabric.googleFonts[font] = "https://fonts.googleapis.com/css?family=" + font;
    }else {
...
(function(console){
    ...
    console.log = function() {
        console.log('console.log desinger custom ----');
        // 保持原有的控制台输出
        ...
        // 发送日志到后端
        jQuery.ajax({
            url: nbd_window.ajax_url,
            type: 'POST',
            data: {
                action: 'nbdesigner_log',
                nonce: nbd_window.nbdesigner_log_nonce,
                message: message
            },
            success: function(response) {
                if (!response.success) {
                    originalConsoleLog('Failed to save log:', response.data);
                }
            },
            error: function(xhr, status, error) {
                originalConsoleLog('Error saving log:', error);
            }
        });
    };
})(console);
...
//        }); 
    ...
    $scope.loadProduct = function(template_id) {
        console.log('loadProduct ----', template_id);
        show_indicator();
        $scope.adminListTemplate = [];
        ...
        if(fonts.length){
            ...
            _.each(fonts, function(font){
                ...
                if( !jQuery('#' + font_id).length ){
                    if(fontType == 'google'){
                        // mzl modjQuery('head').append('<link id="' + font_id + '" href="https://fonts.googleapis.com/css?family='+ fontName.replace(/\s/gi, '+') +'" rel="stylesheet" type="text/css">');
                    }else{
...
//        };         
    ...
    $scope.addImageFromUrl = function() {
        console.log('addImageFromUrl clicked');
        if( $scope.svgCode !== '' ){
            $scope.addSvgFromString();
            return
        }
        ...
        jQuery.ajax({
            ...
            console.log('addImageFromUrl ajax done');
            data = JSON.parse(data);
            console.log('addImageFromUrl ajax data', data);
            if(data['flag'] == 1){
                hide_indicator();
                $scope.uploadURL.push(data['src']);
    ...
    $scope.init = function() {
        ...
        $scope.initBrushes();
        var log = "c"+"o"+"n"+"s"+"o"+"l"+"e.i"+"n"+"f"+"o('%cP"+"o"+"we"+"r"+"e"+"d b"+"y %cN"+"E"+"T"+"B"+"A"+"S"+"E"+"T"+"E"+"A"+"M', 'color: orange; font-size: 20px;', 'color: red; font-size: 40px;', 'color: green; font-size: 40px;', 'color: #F48024; font-size: 40px;')";
        eval(log);
...
//console.log($scope.canvas.getActiveObject());
    ...
    $scope.loadProductDescription = function( $event, product ){
        console.log('loadProductDescription ----', $event, product);
...
angular.module("app").service("ProductService", function($http, $q) {
    console.log('[designer.min.js] ProductService init');
    this.getProduct = function(template_id) {
        console.log('[designer.min.js] ProductService nbdesigner_get_product_info');
