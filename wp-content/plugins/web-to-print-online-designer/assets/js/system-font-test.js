// 系统字体加载测试脚本
console.log('=== 系统字体加载测试开始 ===');

// 测试1: 检查系统字体列表
console.log('测试1: 检查系统字体列表');
var systemFonts = ['Arial', 'Helvetica', 'Times New Roman', 'Georgia', 'Verdana', 'Tahoma', 'Courier New'];
console.log('系统字体列表:', systemFonts);
...
if (typeof angular !== 'undefined') {
    ...
    if (scope && scope.addText) {
        ...
        // 测试添加系统字体文本
        systemFonts.forEach(function(fontName, index) {
        ...
        newFonts.forEach(function(fontName, index) {
            ...
            try {
                scope.addText(fontName + '字体测试文本', 'bodytext', {
                    fontFamily: fontName,
                    fontSize: 24,
                    top: 100 + (systemFonts.length + index) * 50,
                    left: 100
                });
                console.log('✅ ' + fontName + '字体文本添加成功');
            } catch (error) {
...
// 测试4: 检查错误日志
console.log('\n测试4: 检查错误日志');
console.log('请检查控制台是否有 "Fail to load font:" 错误');
console.log('如果修复成功，应该看到 "使用系统字体:" 的日志');
...
if (typeof angular !== 'undefined') {
    ...
    if (scope && scope.insertTemplateFont) {
        ...
        // 测试加载系统字体
        systemFonts.forEach(function(fontName) {
...
if (typeof FontFaceObserver !== 'undefined') {
    ...
    // 测试FontFaceObserver加载系统字体
    systemFonts.forEach(function(fontName) {
        console.log('测试FontFaceObserver加载' + fontName + '字体...');
        try {
            var font = new FontFaceObserver(fontName);
            font.load('Sample Text', 3000).then(function() {
                console.log('✅ FontFaceObserver成功加载' + fontName + '字体');
            }).catch(function(error) {
                console.log('❌ FontFaceObserver加载' + fontName + '字体失败:', error);
            });
        } catch (error) {
