...
function addBackgroundStyles() {
    const styles = `
        ...
        #tab-background .tab-main {
            flex: 1 !important;
            overflow: auto !important;
            padding-top: 10px !important;
        }

        #tab-background .nbd-tab-contents {
            height: 100% !important;
        }

        #tab-background .nbd-list-items {
            display: flex !important;
            flex-direction: column !important;
            width: 100% !important;
        }

        #tab-background .list-item {
            display: flex !important;
            width: 100% !important;
            padding: 10px !important;
        ...
        #tab-background .list-item:hover {
            background-color: #f5f5f5 !important;
        }

        #tab-background .main-item {
            display: flex !important;
            align-items: center !important;
            width: 100% !important;
        }

        #tab-background .item-icon {
            margin-right: 10px !important;
            width: 24px !important;
            text-align: center !important;
        }

        #tab-background .item-text {
            flex: 1 !important;
            font-size: 14px !important;
        }
        
        #tab-background .nbd-tab-content {
            padding: 15px;
        }

        #tab-background .content-item {
            width: 100%;
        }

        #tab-background .nbd-color-palette {
            display: flex;
            flex-direction: column;
            gap: 15px;
        }

        #tab-background .nbd-color-main {
            display: flex;
            align-items: center;
            gap: 10px;
        }

        #tab-background .nbd-preset-colors {
            display: flex;
            flex-wrap: wrap;
            gap: 10px;
        }

        #tab-background .preset-color {
            width: 30px;
            height: 30px;
            border-radius: 4px;
            cursor: pointer;
        ...
        #tab-background .nbd-upload-zone {
        ...
        #tab-background .nbd-upload-zone:hover {
            border-color: #999;
        }

        #tab-background .gradient-tool {
            display: flex;
            flex-direction: column;
            gap: 15px;
        }

        #tab-background .pattern-list {
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(80px, 1fr));
            gap: 10px;
        }

        #tab-background .pattern-item {
            cursor: pointer;
            border: 1px solid #ddd;
            padding: 5px;
        }

        #tab-background .pattern-item:hover {
            border-color: #999;
        }
    `;
    ...
    window.initModernLayout = function(){
        ...
        // if(width > 767 && width <= 1200){
            jQuery('.main-tabs .tab').on('click', function(){
                ...
                console.log('.main-tabs .tab click');
                const sidebar = document.querySelector('.tabs-content');
                const previewButton = document.querySelector('.nbd-show-3d-preview');
                const mainPanel = document.querySelector('.main');
                if(mainPanel && sidebar && width <= 1200) {
                ...
                console.log('previewButton contains(is_open)');// + previewButton.classList.contains('is_open')
            });
            jQuery('.hide-tablet').on('click', function(){
                jQuery('.nbd-sidebar').removeClass('is_open');
                const mainPanel = document.querySelector('.main');
                const previewButton = document.querySelector('.nbd-show-3d-preview');
                if(mainPanel && width <= 1200 && !previewButton.classList.contains('is_open')) {
