...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', 'filterBackgroundFilter', 'filterShapeFilter', 'filterIconFilter', '$timeout', '$http', '$document', '$interval',
    function($scope, FabricWindow, $window, NBDDataFactory, filterFontFilter, filterArtFilter, filterBackgroundFilter, filterShapeFilter, filterIconFilter, $timeout, $http, $document, $interval) {
    $scope.stages = [];
    var d = new Date(),
    t = d.getTime();
    ...
    $scope.iconScrollTop = 0;
    $scope.isIconPageFirstIn = true;
    $scope.defaultConfig = {
        name: 'Typography',
        width: 200,
        height: 200,
    ...
    $scope.initSettings = function(){
        ...
        $scope.resource = {
            ...
            shape: {filter: {perPage: 30, currentPage: 1, totalPage: 20}, data: [], init: true, onload: false, progressBar: 0, ossNextToken: '', hasMore: 'true', filteredShapes: []},
            icon: {filter: {perPage: 30, currentPage: 1, totalPage: 20, currentCat: null}, data: [], cat: [], init: true, onload: false, first: true},
    ...
    $scope.openSubPage = function(type) {
        ...
        console.log('openSubPage --- type:', type, ' resource.shape.data:', $scope.resource.shape.data);
        console.log('openSubPage --- type:', type, ' resource.icon.data:', $scope.resource.icon.data);
    ...
    $scope.closeSubPage = function() {
        ...
        if (iconContainer != null) {
            iconContainer.removeEventListener('scroll', $scope.handleIconScroll);
            $scope.iconScrollContainer = null;
        }
        jQuery('#nbd-icon-wrap .mansory-wrap').masonry('destroy');
        $scope.isIconPageFirstIn = false;
    };
...
/*                 if( angular.isDefined( data.area ) ){
    ...
    $scope.renderMasonryList = function(type, container, item, scrollContainer, init){  
        ...
        if (type == 'icon') {
            console.log('renderMasonryList begin-- containerscrollTop: ', document.querySelector('.icon-subpage-content').scrollTop);
        ...
        imagesLoaded( jQuery(container), function() {
            ...
            //         // console.log('renderMasonryList layoutComplete scope.scrollTop: ', $scope.iconScrollTop, ' currentPage: ', $scope.resource[type].filter.currentPage);
            //         //$scope.iconScrollContainer.scrollTop = $scope.iconScrollTop;
            //         // document.querySelector('.icon-subpage-content').scrollTop = $scope.iconScrollTop;
            //         //$scope.iconScrollContainer.scrollTo(0, $scope.iconScrollTop);
            ...
            //todo 6.27
            console.log('renderMasonryList layoutComplete containerscrollTop: ', $scope.iconScrollTop);
            if (type === 'icon') {
                document.querySelector('.icon-subpage-content').scrollTop = $scope.iconScrollTop;//document.querySelector('.icon-subpage-content').scrollHeight;
            }
            var count = 0;
    ...
    $scope.bindIconScrollEvent = function() {
        var iconContainer = document.querySelector('.icon-subpage-content');
        if ($scope.iconScrollContainer == null && iconContainer) {
            ...
            // 添加新的滚动事件监听器
            iconContainer.addEventListener('scroll', $scope.handleIconScroll);
            
            // 存储容器引用
            $scope.iconScrollContainer = iconContainer;
        } else {
    ...
    // 处理图标页面滚动事件
    $scope.handleIconScroll = function(event) {
        ...
        if (scrollTop + clientHeight >= scrollHeight - 20) {
            ...
            $scope.iconScrollTop = scrollTop + clientHeight;
            $scope.scrollLoadMore('#tab-icon', 'icon');
            // $scope.loadOSSFiles('icon', container);
            // container.scrollTop = scrollTop;
        } else {
            $scope.iconScrollTop = scrollTop;
        }
    ...
    $scope.scrollLoadMore = function(container, type){
        console.log('scrollLoadMore type: ', type, ' container: ', container, ' oss onload: ', $scope.resource[type].onload);
        if( $scope.resource[type].onload ) return;
...
nbdApp.filter("filterIcon", function() {
    return function(icons, filterIcon) {
        console.log('filterIcon --- icons:', icons);
        console.log('filterIcon --- filterIcon:', filterIcon);

        var arrIcons = [];
        icons = _.sortBy(icons, [function(o) { return o.key; }]);
        angular.forEach(icons, function(icon, key) {
            // var check = [];
            // check['limit'] = arrIcons.length > ( filterIcon.perPage * filterIcon.currentPage  - 1 ) ? false : true;
            // if( !!filterIcon.search ){
            //     check['name'] = icon.key.toLowerCase().indexOf(filterIcon.search.toLowerCase()) >= 0 ? true : false;
            // }else{
            //     check['name'] = true;
            // };
            // if ( check['limit'] && check['name'])
...
nbdApp.filter("filterShape", function() {
    return function(shapes, filterShape) {
        console.log('filterShape --- shapes:', shapes);
        console.log('filterShape --- filterShape:', filterShape);

        var arrShapes = [];
        shapes = _.sortBy(shapes, [function(o) { return o.key; }]);
        angular.forEach(shapes, function(shape, key) {
            var check = [];
            check['limit'] = arrShapes.length > ( filterShape.perPage * filterShape.currentPage  - 1 ) ? false : true;
            if( !!filterShape.search ){
                check['name'] = shape.key.toLowerCase().indexOf(filterShape.search.toLowerCase()) >= 0 ? true : false;
            }else{
                check['name'] = true;
            };
            if ( check['limit'] && check['name'])
