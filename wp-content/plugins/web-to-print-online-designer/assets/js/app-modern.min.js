...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', 'filterBackgroundFilter', '$timeout', '$http', '$document', '$interval',
    ...
    $scope.initSettings = function(){
        ...
        $scope.$watchCollection('resource.background.filter', function(newVal, oldVal){
            ...
            $scope.resource.background.filteredBackground = filterBackgroundFilter($scope.resource.background.data.bgs, $scope.resource.background.filter);
            if(newVal.search != oldVal.search){
                $timeout(function() {
                    jQuery("#tab-svg .tab-scroll").stop().animate({
                        scrollTop: 0
...
nbdApp.filter("filterBackground", function() {
    return function(bgs, filterBackground) {
        ...
        var arrBgs = [];
        bgs = _.sortBy(bgs, [function(o) { return o.name; }]);
        angular.forEach(bgs, function(bg, key) {
            ...
            if( !!filterBackground.search ){
               check['name'] = bg.name.toLowerCase().indexOf(filterBackground.search.toLowerCase()) >= 0 ? true : false;
            }else{
                check['name'] = true;
            };
            // if( !!filterArt.currentCat.id || filterArt.currentCat.id === 0 ){
