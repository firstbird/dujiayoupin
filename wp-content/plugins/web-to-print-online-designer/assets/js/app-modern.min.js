...
/*         var _canvas = this.stages[stage_id]['canvas'];
    ...
    // $scope.updateRuler = function() {
    //     console.log('updateRuler ----- ');
    //     // 移除旧的标尺（如果存在）
    //     var stage = $scope.stages[$scope.currentStage];
    //     if (stage && stage.canvas) {
    //         // var oldRuler = stage.canvas.wrapperEl.querySelector('canvas:not(#designer-canvas)');
    //         // if (oldRuler) {
    //         //     oldRuler.remove();
    //         // }
    ...
    $scope.addCanvasRuler = function() {
        ...
        function drawRuler() {
            ...
            for (let x = 0; x <= canvas.width; x += PIXELS_PER_CM) {
                // 转换为厘米
                const cm = Math.round(x / PIXELS_PER_CM * 10) / 10;
                ...
                // 添加数字标签
                ctx.fillText(cm + 'cm', x, 20);
                
                // 绘制小刻度 (每毫米一个)
                for (let mm = 1; mm < 10; mm++) {
                    const xMm = x + (PIXELS_PER_CM / 10 * mm);
                    if (xMm < canvas.width) {
                        ctx.beginPath();
                        ctx.moveTo(xMm, 0);
                        ctx.lineTo(xMm, 5);
            ...
            for (let y = 0; y <= canvas.height; y += PIXELS_PER_CM) {
                // 转换为厘米
                const cm = Math.round(y / PIXELS_PER_CM * 10) / 10;
                ...
                // 绘制小刻度 (每毫米一个)
                for (let mm = 1; mm < 10; mm++) {
                    const yMm = y + (PIXELS_PER_CM / 10 * mm);
                    if (yMm < canvas.height) {
                        ctx.beginPath();
                        ctx.moveTo(0, yMm);
                        ctx.lineTo(5, yMm);
                        ctx.stroke();
        ...
        drawRuler();
        // 创建一个覆盖层来显示刻度
        var rulerOverlay = document.createElement('canvas');
        ...
        // 将覆盖层添加到 canvas 容器
        canvas.wrapperEl.appendChild(rulerOverlay);
        
        // 在覆盖层上绘制刻度
        var rulerCtx = rulerOverlay.getContext('2d');
        
        // 绘制刻度
        function drawRulerOnOverlay() {
            var stage = $scope.stages[$scope.currentStage];
            if (!stage || !stage.canvas) return;
            
            var canvas = stage.canvas;
            var rulerOverlay = document.createElement('canvas');
            rulerOverlay.width = canvas.width + 30;  // 额外空间用于显示刻度
            rulerOverlay.height = canvas.height + 30;
            ...
            var ctx = rulerOverlay.getContext('2d');
            
            // 清除之前的绘制
            ctx.clearRect(0, 0, rulerOverlay.width, rulerOverlay.height);
            ...
            ctx.fillRect(0, 0, rulerOverlay.width, 30);  // 水平标尺背景
            ctx.fillRect(0, 0, 30, rulerOverlay.height); // 垂直标尺背景
            ...
            // 绘制水平刻度
            for (let x = 30; x <= canvas.width + 30; x += PIXELS_PER_CM) {
                // 转换为厘米
                const cm = Math.round((x - 30) / PIXELS_PER_CM * 10) / 10;
                ...
                // 添加数字标签
                ctx.fillStyle = '#333';
                ctx.fillText(cm, x, 15);
                
                // 绘制小刻度 (每毫米一个)
                for (let mm = 1; mm < 10; mm++) {
                    const xMm = x + (PIXELS_PER_CM / 10 * mm);
                    if (xMm < canvas.width + 30) {
                        ctx.beginPath();
                        ctx.moveTo(xMm, 27);
                        ctx.lineTo(xMm, 30);
                        ctx.strokeStyle = '#666';
            ...
            // 绘制垂直刻度
            ctx.textAlign = 'right';
            for (let y = 30; y <= canvas.height + 30; y += PIXELS_PER_CM) {
                // 转换为厘米
                const cm = Math.round((y - 30) / PIXELS_PER_CM * 10) / 10;
            ...
            function drawGuidelines() {
                ...
                ctx.beginPath();
                ctx.moveTo(30, 30);
                ctx.lineTo(rulerOverlay.width, 30);
                ctx.strokeStyle = '#999';
                ctx.stroke();
                ...
                ctx.beginPath();
                ctx.moveTo(30, 30);
                ctx.lineTo(30, rulerOverlay.height);
                ctx.strokeStyle = '#999';
                ctx.stroke();
            }
            ...
            ctx.font = '9px Arial';
            ctx.textAlign = 'left';
            ctx.fillText('cm', 5, 10);
            
            // 添加PPI信息
            ctx.fillText('300 PPI', 5, 20);
