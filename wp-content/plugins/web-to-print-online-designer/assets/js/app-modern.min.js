...
//            }
    ...
    $scope.insertTemplateFont = function(font_name, callback) {
        console.log('=== 新的insertTemplateFont函数被调用 font_name === ', font_name, ' callback: ', callback);
        // var font_id = font_name.replace(/\s/gi, '').toLowerCase();
        // font_name = font_id;
        // 新增的本地字体映射
        // var newLocalFonts = ['greatvibes', 'hindsiliguri', 'lustria'];
        
        // 检查是否为新增的本地字体
        // if (newLocalFonts.indexOf(font_name) !== -1) {
            // console.log('检测到新增本地字体:', font_name);
            
            // 注入字体CSS
            var existingCSS = jQuery('#' + font_name);
            
            if (!existingCSS.length) {
                var localPath = '/wp-content/plugins/web-to-print-online-designer/assets/fonts/' + font_name + '.ttf';
                var css = "<style type='text/css' id='" + font_name + "'>";
                css += "@font-face {";
                ...
                console.log('字体CSS已存在:', font_name);
            }
        ...
        /*
        // 对于其他字体，使用原始逻辑
        console.log('使用原始字体加载逻辑:', font_name);
        ...
        if (existingFont) {
            ...
            if (existingFont.type === 'google') {
                // Google字体处理
                console.log('处理Google字体:', font_name);
                var font_id = font_name.replace(/\s/gi, '').toLowerCase();
                var existingCSS = jQuery('#' + font_id);
                ...
                if (!existingCSS.length) {
                    ...
                    jQuery('head').append('<link id="' + font_id + '" href="' + googleFontUrl + '" rel="stylesheet" type="text/css">');
                    console.log('Google字体CSS已添加:', googleFontUrl);
                }
                
                // 对于Google字体，使用FontFaceObserver确保加载
                if (typeof FontFaceObserver !== 'undefined') {
                    var font = new FontFaceObserver(font_name);
                    font.load('Sample Text', 3000).then(function() {
                        console.log('✅ Google字体加载成功:', font_name);
                        if (callback) callback(font_name);
                    }).catch(function(error) {
                        console.log('⚠️ Google字体加载失败，继续使用:', font_name, error);
                        if (callback) callback(font_name);
                    });
                } else {
                    console.log('FontFaceObserver不可用，直接使用Google字体:', font_name);
                    if (callback) callback(font_name);
                }
                return;
            } else {
                ...
                if (!existingCSS.length) {
                    ...
                    font_url = localFontPaths[0];
                    console.log('使用本地字体路径:', font_url);
                ...
                // 直接调用回调，避免FontFaceObserver超时
                console.log('✅ 字体准备完成:', font_name);
            ...
            console.log('字体不存在于资源中，尝试本地字体映射');
            ...
            if (localFontMap[font_name]) {
                var fileName = localFontMap[font_name];
                var localPath = '/wp-content/plugins/web-to-print-online-designer/assets/fonts/' + fileName;
                
                console.log('使用本地字体映射:', font_name, localPath);
                ...
                if (!existingCSS.length) {
                    var format = fileName.endsWith('.ttf') ? 'truetype' : 'woff';
                    var css = "<style type='text/css' id='" + font_id + "'>";
                    css += "@font-face {font-family: '" + font_name + "';";
                ...
                // 直接调用回调，不等待FontFaceObserver
                console.log('✅ 本地字体准备完成:', font_name);
                ...
                _.each(fonts, function(font, index){
                    ...
                    if( index == fonts.length - 1 ){
                        $scope.insertTemplateFont(font.alias, function(font_name){
                            console.log('insertTemplateFont callback1: ', font_name);
                            // $scope.addFontText(font_name);
                            loadLayers();
                        ...
                        $scope.insertTemplateFont(font.alias, function(font_name){
                            console.log('insertTemplateFont callback2: ', font_name);
                            // $scope.addFontText(font_name);
                            if(!_.filter(stage.states.usedFonts, ['alias', font_name]).length){
                                stage.states.usedFonts.push($scope.getFontInfo(font_name));
                            };
