...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', 'filterBackgroundFilter', '$timeout', '$http', '$document', '$interval',
    ...
    $scope.initSettings = function(){
        ...
        $scope.resource = {
            ...
            background: {filter: {perPage: 10, currentPage: 1, total: 3, totalPage: 10}, data: [], onload: false, init: true, ilr: false, progressBar: 0, filteredBackground: []},
        ...
        $scope.$watch('resource.background', function(newVal) {
            console.log('background data:', newVal);
            console.log('filteredBackground:', newVal.filteredBackground);
            console.log('currentPage:', newVal.filter.currentPage);
            console.log('perPage:', newVal.filter.perPage);
            console.log('total:', newVal.filter.total);
            console.log('onload:', newVal.onload);
...
/*                 if( angular.isDefined( data.area ) ){
    ...
    // 添加OSS文件加载方法
    $scope.loadOSSFiles = function(type = '', container = '', callback, token = '') {
        var self = this;
        console.log('PhotoAlbum: 加载OSS文件 ----', token);
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    self.ossNextToken = response.data.nextToken;
                    ...
                    if (response.data.hasMore) {
                        $('#nbd-load-more-files')
                            ...
                            .off('click')
                            .on('click', function() {
                                self.loadOSSFiles(self.ossNextToken);
                            });
                    } else {
