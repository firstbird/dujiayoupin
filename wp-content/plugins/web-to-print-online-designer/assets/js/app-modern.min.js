...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', 'filterBackgroundFilter', '$timeout', '$http', '$document', '$interval',
    function($scope, FabricWindow, $window, NBDDataFactory, filterFontFilter, filterArtFilter, filterBackgroundFilter, $timeout, $http, $document, $interval) {
    $scope.stages = [];
    var d = new Date(),
    t = d.getTime();
    ...
    $scope.getMedia = function(type, context){
        ...
        switch( type ){
            ...
            case 'storyset':
                ...
                break;
            default:
                console.log('getMedia default: ', type, category, search);
                $http({
                    method: 'GET',
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    if (response.data.length !== 0) {
                        ...
                        console.log('loadOSSFiles response before --- filter:', $scope.resource[type].filter);
                        $scope.resource[type].filter.total = $scope.resource[type].data.length;
                        console.log('loadOSSFiles response after --- filter:', $scope.resource[type].filter);
                        if( callback ) $scope.afterGetResource(type);
