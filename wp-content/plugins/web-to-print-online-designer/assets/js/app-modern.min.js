        ...
        if(!_.filter($scope.resource.font.data, ['alias', font_name]).length){
            ...
            if (typeof FontFaceObserver !== 'undefined') {
                ...
                function attemptFontLoad() {
                    ...
                    var timeout = 8000 + (retryCount * 4000);
    ...
    $scope.insertTypography = function(typo){
        ...
        NBDDataFactory.get('nbd_get_resource', {type: 'get_typo', folder: typo.folder}, function(data){
            ...
            function loadLayersWithFontEffects(){
                console.log('🎨 开始加载字体模板图层，确保艺术效果立即渲染');
        ...
        setTimeout(function() {
            ...
            if(fonts.length){
                console.log('🎨 开始加载字体，数量:', fonts.length);
                 console.log('📊 字体加载开始时间:', new Date().toISOString());
                ...
                Promise.all(fontLoadPromises).then(function(loadedFonts) {
                    ...
                    console.log('📊 字体加载总耗时:', typographyLoadDuration.toFixed(2), 'ms');
                    console.log('📊 字体加载完成时间:', new Date().toISOString());
                    console.log('📊 字体加载方式: Promise.all 成功');
                    
                    // 智能等待字体完全生效
                    waitForFontsToLoad(loadedFonts, function() {
                        console.log('⏰ 字体完全生效，开始创建文本对象');
                        console.log('📊 字体生效时间:', new Date().toISOString());
                    ...
                    console.log('📊 字体加载失败耗时:', typographyLoadDuration.toFixed(2), 'ms');
                    console.log('📊 字体加载失败时间:', new Date().toISOString());
                    console.log('📊 字体加载方式: Promise.all 失败，使用回退机制');
